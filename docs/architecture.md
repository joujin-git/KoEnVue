...
| [Core/Color/ColorHelper](../Core/Color/ColorHelper.cs) | Hex ↔ COLORREF ↔ RGB ↔ ARGB conversion. `HexToArgb` 는 `0xFFRRGGBB` 불투명 ARGB (커서 헤일로 셰이더가 픽셀 채널을 직접 쓸 때 — `HexToColorRef` 의 BGR 형제). Malformed hex returns 0 / `(0,0,0)` / `0xFF000000` instead of throwing, so a bad `config.json` doesn't leak GDI handles on the render hot path | `TryNormalizeHex`, `HexToColorRef`, `HexToArgb`, `HexToRgb`, `ColorRefToRgb`, `RgbToHex` |
