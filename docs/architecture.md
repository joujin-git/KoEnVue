...
| [App/UI/Dialogs/SettingsDialog.cs](../App/UI/Dialogs/SettingsDialog.cs) (+ `.Layout.cs` + `.Fields.cs` + `.Scroll.cs`) | Scrollable settings dialog (PR-21 14 sections / 72 필드 — 일반 / 메인 인디 / 커서 인디 3 대분류 재배치. 커서 인디 = "커서 인디 — 동심원" 10 필드 + "커서 인디 — 전환 효과" 2 필드, 모두 DefaultConfig MinCursor*/MaxCursor* const 참조. `cursor_change_highlight` on/off 는 트레이 메뉴 전용이라 미노출) split across 4 partial class files. `BuildChildren` 은 레이아웃 계산을 [`SettingsDialog.Layout.cs`](../App/UI/Dialogs/SettingsDialog.Layout.cs) 의 `internal readonly record struct SettingsLayout`(27필드 — 18 DPI-스케일 메트릭 + ClientW/H + 뷰포트 지오메트리 + 파생 열 좌표·clamp 컨트롤 폭) + `BuildLayout(ctx)` 순수 팩토리로 분리하고, 자신은 그 메트릭으로 윈도우를 만드는 5종 헬퍼(`RegisterViewportClass`/`CreateDescriptionLabel`/`CreateViewport`/`LayoutRows`/`CreateButtons`)를 오케스트레이션한다 (IMP-2 분해, 2026-06-02 — `SettingsLayout` = 레이아웃 메트릭 단일 진실원, 픽셀 동등 보존, `SettingsLayoutTests` 가 파생식 박제). cross-file static 부작용(`_lineHeight`/`_viewportClientH`/`_scrollMax`)은 BuildChildren 에 집중 (헬퍼는 값만 반환) |
