...
| [App/UI/TrayIcon.cs](../App/UI/TrayIcon.cs) | GDI-based dynamic icon bitmap — **헤일로 링 + 배지** 디자인(제품의 두 표시 요소 은유). IME 상태별 배경색·도형색을 `CreateIcon`이 즉석에서 그려 `SafeIconHandle`로 반환. **숨김 표시는 보이는 도형만 그리기** — 링(반지름 `min(W,H)/2-1` · 두께 `min(W,H)/8`, Fg 원 채운 뒤 안쪽을 배경색으로 파냄) + 배지(`iconW/3` × `iconH/5`, 링 안쪽 중앙). 4상태 전부 구별(링+배지 / 배지 / 링 / 없음), 도형 선택은 `Tray.GetShapes`. 치수는 16px 실측(inset 0 이면 모서리에서 원이 잘리고, 배지 `3/8` 폭이면 링에 닿음). 앱 아이콘 `koenvue.ico` 도 동일 도형 8해상도 |
