...
| [App/UI/CursorRenderer.cs](../App/UI/CursorRenderer.cs) | 커서 추종 인디의 distance-field 분석적 AA 픽셀 셰이더. `LayeredCursorBase` 의 `renderToDib` 콜백으로 주입되어 `CursorStyle` + `CursorMetrics` 를 받아 DIB BGRA32 픽셀에 직접 쓴다. 평상시(soft=0) 각 동심원 = 코어 (사용자 색상) + 헤일로 (흰색 × `HaloOpacity`). CAPS OFF 시 Outer skip. **PR-21** — 반지름·두께 ×`HighlightScale`. **PR-29** — `MotionSoftness`&gt;0 이면 하드 코어 없이 가우시안 안개(σ≈baseHalo×14, 색+흰색 혼합) + `RingAlpha*` 스케일; Outer는 CAPS ON 일 때만 |
| [App/UI/CursorMotionDim.cs](../App/UI/CursorMotionDim.cs) | PR-29 이동 딤 **순수** 상태 전이 (Win32/엔진 무의존). `AdvanceDimActive`(enter 즉시 / exit settle 틱 히스테리시스) · `EffectiveSoftness`(팝·비활성·비딤 → 0) · `RingAlphas`(딤 시 motionAlpha×배수, 아니면 1). 창 `SourceConstantAlpha` 는 `FullAlpha=255` — 딤은 셰이더만. 단위 `CursorMotionDimTests` |
| [App/UI/CursorOverlay.cs](../App/UI/CursorOverlay.cs) | `LayeredCursorBase` 위의 정적 파사드 — `Initialize(hwnd, hwndTimer, config, state, caps)` / `HandleConfigChanged` / `HandleCursorMotionTimer` / `HandleCursorPopTimer` / `SetImeState` / `SetCapsLock` / `Dispose`. **PR-21 IME 전환 스케일 팝 상태머신** — `TriggerPop` / `HandleCursorPopTimer` / `StopPop` + `_hwndTimer` / `_popActive` / `_popStartTick`. `SetImeState` 가 가시 + `AnimationEnabled && CursorChangeHighlight` 면 `TriggerPop`, 아니면 색만 즉시 `Render`. 팝은 IME 상태가 실제 바뀔 때만. **PR-29 이동 딤** — AlwaysShow 모드에서 맨해튼 δ로 딤 enter/exit(`CursorMotionDim`) → `MotionSoftness`/`MotionFogPadLogicalPx`/`RingAlpha*` 배선 후 재렌더; 팝 중 딤 억제; 창 알파는 Full. `Initialize` 가 커서 좌표 시드. **`BuildStyle`** 가 IME+CAPS → `CursorStyle` 합성 단일 진입점. 정지 검출 FSM 또는 AlwaysShow(`CursorAlwaysPollMs=15`) 분기. 별도 HWND + `WS_EX_TRANSPARENT`. **셸 UI 호버 숨김** — `IsOverShellUi` |
