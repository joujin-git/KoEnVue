...
Detection thread (BG):
  ...
  • SystemFilter evaluation (8 hide conditions)
...
│   ├── Logging/             Logger + LogLevel + ILogSink / LogProvider
...
│   ├── Shell/               UriLauncher (ShellExecuteW open)
│   ├── Xml/                 XmlEntityCodec (schtasks XML entity escape)
...
│   │                        PortablePath, CursorDisplayModeMigration
│   ├── Detector/            ImeStatus + SystemFilter + ImeConstants +
│   │                        OverlaySuppressProbe (PR-32 WFP 축) +
│   │                        DetectionService + DetectionHost (감지 스레드)
...
│       └── Dialogs/         CleanupDialog + ScaleInputDialog + SettingsDialog
│                            (~69 Add 필드 / 14 섹션)
│
├── Program.cs               Main message loop + WndProc. Detection 루프는
│                            App/Detector/DetectionService 위임. Bootstrap 순서·
│                            hwnd volatile (PR-18) 은 기존과 동일. 설정 교체를 앱
│                            전역에 반영하는 것은 ApplyConfigTransition 단일 구현 —
│                            진입점 2개 (HandleConfigChanged 핫리로드 · SaveAndSync
│                            저장 중 3-way 병합). WM_DESTROY 가 파괴된 창의 hwnd
│                            필드를 즉시 Zero 로 내린다 (G5).
...
└── tests/KoEnVue.Tests/     xUnit (dev-only, P1 예외). InternalsVisibleTo.
    └── Unit/                baseline **236 PASS** / **27 파일** (2026-08-03)
        ...
        ├── SaveMergeTests / WindowLifecycleTests / IndicatorVisibilityTests
...
|--------|---------|----------------|
| [Core/Native/*](../Core/Native/) | Raw P/Invoke surface. `Win32Types.cs` centralizes every struct + the `Win32Constants` class (SM/WS/DWMWA/MB_*/etc. — `MB_OK = 0x00000000` 은 PR-15 후속 fix #2 (admin → 일반 down-grade) 가 `User32.MessageBoxW` 의 `uType: 0` hard-code 정리 + 신규 down-grade 안내 호출 site 통일을 위해 추가; `LOWORD_MASK = 0xFFFF` (WM_COMMAND/WM_VSCROLL LOWORD) + `KEY_PRESSED = 0x8000` (`GetAsyncKeyState` 눌림 비트) 은 감사 묶음 1 (2026-06-02) 이 다이얼로그/드래그 modifier 의 인라인 매직 넘버를 단일화한 자리 — `KEY_PRESSED` 의 Core 배치는 `Program.IsDragModifierPressed` (App) 와 `LayeredOverlayBase` Shift 감지 (Core) 양쪽 공유라 P4 정합). `SafeGdiHandles.cs` hosts `SafeFontHandle`, `SafeIconHandle`, etc. `WinHttp.cs` hosts `SafeWinHttpHandle : SafeHandleZeroOrMinusOneIsInvalid` (선언만 있고 사용처 0 인 const 4개는 사용 중 상수의 짝/대안으로 의도적 보존 — 감사 HC-17). **(미래 UIA 도입 전제이던 `OleAut32.cs` (`SysFreeString` + SafeArray 5종) 는 사용처 0 으로 감사 묶음 1 에서 삭제 — 재도입 시 복원.)** | `[LibraryImport]` only, no `[DllImport]` |
...
| [Core/Color/ColorHelper](../Core/Color/ColorHelper.cs) | Hex ↔ COLORREF ↔ RGB ↔ ARGB conversion. `HexToArgb` 는 `0xFFRRGGBB` 불투명 ARGB (커서 헤일로 셰이더가 픽셀 채널을 직접 쓸 때 — `HexToColorRef` 의 BGR 형제). Malformed hex returns 0 / `(0,0,0)` / `0xFF000000` instead of throwing, so a bad `koenvue_config.json` doesn't leak GDI handles on the render hot path | `TryNormalizeHex`, `HexToColorRef`, `HexToArgb`, `HexToRgb`, `ColorRefToRgb`, `RgbToHex` |
...
| [Core/Http/HttpClientLite](../Core/Http/HttpClientLite.cs) | Synchronous HTTPS GET wrapper backed by WinHTTP. NativeAOT publish impact ~40 KB (vs ~2.5 MB for `System.Net.Http.HttpClient`). Response body cap 256 KB, all failure paths return `null`. 비-200·일시 예외는 `LogProvider.Sink?.Debug` (감사 후속 P3 — `HttpStatusOk` const) | `GetString(userAgent, host, path, extraHeaders?, timeoutMs = 10_000) → string?` |
| [Core/Logging/Logger](../Core/Logging/Logger.cs) + `LogLevel` | Async file logger. `ConcurrentQueue` + `ManualResetEventSlim` + dedicated drain thread. Single `.log → .log.old` rotation. **No `AppConfig` parameter** — Stage 3-A narrowed `Initialize` to primitives. **Queue cap** `MaxQueueSize = 10_000`: 회전 실패로 `_fileWriter = null` 상태가 지속되면 최고령부터 드롭하고 복구 시 누적 드롭 건수를 1회 경고로 기록 (무제한 성장 방지). **라우팅** `FileLogRoute { PreInit, Queue, Drop }` + `volatile _route` (bug-hunt 2026-08-02 G1·G2): `_drainThread` 필드는 **Join 대상 참조 전용**이고 로그 목적지 판정은 이 enum 단독 — 파일 로깅 OFF 와 초기화 실패(writer 락 타임아웃 / `StreamWriter` 생성 실패)는 `Drop` 이라 소비자 없는 pre-Init 버퍼로 새지 않는다 (`Trace` 출력은 유지). **lifecycle 락** `_lifecycleLock` (G3): `Initialize` / `Shutdown` 을 1초 상한 `TryEnter` 로 직렬화 — `Shutdown` 은 `AppDomain.UnhandledException` 경로로 **임의 스레드**에서 들어오므로 자기-Join 회피 (`thread == Thread.CurrentThread`) 와 writer 락 재진입 감지 (`Monitor.IsEntered`) 도 함께 가드. 획득 순서는 항상 `_lifecycleLock` → `_writerLock` (drain 스레드는 후자만 쓰므로 역순 경로 없음). **레벨 가드** `IsEnabled`: 보간 문자열 합성은 호출 사이트 책임(`ILogSink` 계약)이라, 인자 계산이 비싼 사이트(`GetClassName` 같은 P/Invoke 조회)가 사전 가드로 써서 레벨이 꺼져 있을 때의 비용을 없앤다 | `Initialize(bool enabled, string? logFilePath, int maxSizeMb)`, `SetLevel(LogLevel)`, `IsEnabled(LogLevel)`, `Debug`/`Info`/`Warning`/`Error`, `Shutdown` |
| [Core/Config/JsonSettingsManager\<T\>](../Core/Config/JsonSettingsManager.cs) + [JsonSettingsFile](../Core/Config/JsonSettingsFile.cs) | Generic JSON-backed settings pipeline. `JsonTypeInfo<T>` injection is mandatory under NativeAOT + `JsonSerializerIsReflectionEnabledByDefault=false`. Five `protected virtual` hooks run in fixed order during `Load`: `ApplyNullSafetyNet` → `PostDeserializeFixup` → `Migrate` → `Validate` → `ApplyTheme`. `MergeWithDefaults` (PostDeserializeFixup 구현 — STJ source-gen 이 `init` 기본값을 reflection-off 에서 드롭하는 우회책) 은 **재귀 머지** (P0 fix, 2026-06-01): 양쪽 모두 객체인 키는 `MergeObjects` 로 내려가 사용자가 중첩 객체 (`event_triggers` / `advanced` 등) 를 **부분만** 지정해도 누락 형제 필드가 기본값을 보존 (이전 최상위-키-only 머지는 중첩 객체를 통째 교체 → 누락 형제를 `default(T)` 로 드롭하던 회귀). 배열·Dictionary 는 의도적으로 통째 교체 (사용자의 항목 축소 의도 보존). 사용자 JSON 은 `JsonDocumentOptions{ CommentHandling=Skip, AllowTrailingCommas=true }` 로 파싱해 소스 생성 컨텍스트와 일치 — 주석/트레일링 콤마가 든 **정상** config 가 머지 전처리에서 `JsonException` 으로 "손상" 오판되어 전체 디폴트 묵음 무시되던 회귀 차단. 가시성 `internal` (테스트 노출). Delete-safe hot reload (`File.Exists` pre-check) and corrupted-file spam prevention (mtime cache update inside `catch`) are baked in. **Atomic save**: `WriteAllText` writes to `path + ".tmp"` then `File.Move(tmp, path, overwrite: true)` — `MoveFileExW(MOVEFILE_REPLACE_EXISTING)` guarantees atomic rename on the same volume, so a crash mid-save cannot leave a truncated config file. **Cross-thread mtime 동기화**: `_lastMtime` (DateTime) 은 메인 스레드 (`Load`/`Save`) 와 감지 스레드 (`CheckReload`) 가 공유하는데 64비트 값이라 `volatile` 키워드를 못 받으므로 짧은 `lock (_mtimeLock)` 으로 read/write 의 원자성·가시성을 보장한다 (`GetLastWriteTimeUtc` I/O 는 lock 밖, 필드 대입/비교만 보호) — Logger drain thread / `ModalDialogLoop.s_activeDialog` 와 같은 결의 무동기화 race 차단. **3-way 병합 + 되읽기 실패 롤백**: `Save` 는 디스크 mtime 이 `_syncedMtime` 과 다르면 기준선(`_lastPersistedJson`) 대비 앱 변경분만 얹고 **병합 결과를 되읽어 반환**한다 (병합이 없었으면 입력 인스턴스를 그대로 반환 — 호출자가 참조 비교로 구분). 되읽기가 실패하면 `_lastMtime` / `_syncedMtime` 을 `MinValue` 로, 기준선을 호출자가 실제로 들고 있는 `rawJson` 으로 **함께 물린다** — "동기화됨" 오표시를 남기면 다음 저장이 병합을 건너뛰거나 사용자 편집을 "앱이 바꿨다" 로 오분류해 방금 살아남은 편집을 되돌려 쓴다 (bug-hunt 2026-08-02 G11, `SaveMergeTests`) | `Load() → T`, `Save(T) → T`, `CheckReload() → bool`, `FilePath` |
...
| [Core/Windowing/CursorStyle + CursorMetrics](../Core/Windowing/CursorStyle.cs) | `internal readonly record struct` pair — `LayeredCursorBase` 의 primitive-only 입력 / 출력 경계. `CursorStyle` (engine **input**): Outer/Middle/Inner `*RadiusLogicalPx`, `CoreThicknessLogicalPx` / `HaloThicknessLogicalPx`, `HaloOpacity : double`, 3 색상 `*ColorArgb : uint`, `CapsLockOn : bool`, `HighlightScale : double = 1.0` (PR-21), **PR-29** `MotionSoftness` / `MotionFogPadLogicalPx` / `RingAlphaInner|Middle|Outer`. `BoundingBoxLogicalPx` 헬퍼가 외측 반지름 + 헤일로 외측 확장 + AA 여유 1px 에 `MaxHighlightScale` 을 곱해 DIB 한 변을 잡고, soft&gt;0 안개 시 `MotionFogPadLogicalPx` 를 더해 σ 잘림을 막음 — CAPS 토글·팝·딤 진행에서 같은 bbox 정책. **`public const double MaxHighlightScale = 2.0`** (PR-21) = 팝 상한 = bbox 고정 기준. App 측 `DefaultConfig.MaxCursorHighlightScale` 이 본 Core const 를 참조 (P6 정방향). `CursorMetrics` (3 fields): `DpiScale` + `ScaledWidth` / `ScaledHeight` | — |
...
| [Core/Windowing/ModalDialogLoop](../Core/Windowing/ModalDialogLoop.cs) | Static `Run(hwndDialog, hwndOwner, ref bool isClosedFlag)` replacing the duplicated `EnableWindow(owner, false) + while(GetMessageW... IsDialogMessageW(...)) { Translate/Dispatch } + EnableWindow(owner, true)` boilerplate in all three dialogs. Re-posts `WM_QUIT` when consumed by the nested loop so the outer message loop also terminates. `IsActive` / `ActiveDialog` expose the current modal HWND so non-UI-thread consumers (e.g. `DetectionService`) can gate their own side effects while a dialog is modal — 내부 `s_activeDialog` 필드는 `IntPtr` 라 `volatile` 을 받지 못하므로 모든 접근에 `Volatile.Read` / `Volatile.Write` 를 명시해 교차 스레드 가시성을 보장한다. `RunExternal(hwndSentinel, action)` 는 `MessageBoxW` 처럼 Win32 가 자체 메시지 루프를 돌리는 외부 모달 구간에 `IsActive` 가드만 씌우는 경량 변형 (메시지 펌프/`EnableWindow` 미조작, 중첩 시 이전 값 스택 저장/복원) — see [implementation-notes → Detection → Modal dialog gate](implementation-notes.md#modal-dialog-gate). **`ExtraModalWindows : Func<IntPtr[]>?`** (bug-hunt 2026-08-02 G16) — `EnableWindow(owner, false)` 는 **소유자 하나만** 막으므로, 소유자를 갖지 않는 별도 최상위 창(KoEnVue 의 플로팅 배지: `WS_EX_TOPMOST` / `hWndParent = NULL`)은 모달 중에도 드래그되고 그 드래그 종료가 `Settings.Save` 까지 수행해 **열린 다이얼로그 뒤에서 설정이 갈아치워진다**. App 이 부팅 시 1회 배선하고 `Run` 이 **진입 시점에 조회**해 함께 비활성화한 뒤 finally 에서 같은 목록을 되살린다 (런타임 생성·파괴 안전, 미배선이면 종전 동작). Core 는 App 의 창을 알 수 없으므로(P6) 공급자 형태. 포커스 복원 2 지점(`Run` finally · `RejectReentry`)은 `IsWindowVisible` 가드 — 비가시 소유자(0×0 메시지 전용 창)에 포커스를 주면 감지 루프의 self-HWND 가드에 매 틱 걸려 배지가 되살아나지 않는다 (G10) | `Run`, `RunExternal`, `IsActive`, `ActiveDialog`, `ExtraModalWindows` |
...
| [Core/Windowing/Win32DialogHelper](../Core/Windowing/Win32DialogHelper.cs) | DPI-aware dialog non-client metrics + 9 pt system font helper + dialog position calculator + `WNDCLASSEXW` registration helper. `CreateDialogFont(dpiY, fontFamily) → SafeFontHandle` (PR-08 E3 — 폰트 패밀리는 App 측 [DefaultConfig.DefaultDialogFontFamily](../App/Config/DefaultConfig.cs) 가 주입; Core 는 한국어 폰트 어휘를 알지 않음) 가 `CreateFontW` + `SafeFontHandle` 보일러플레이트를 흡수. `CalculateDialogPosition` unifies the monitor-centered (Cleanup/Settings) and cursor-anchored (ScaleInput) patterns — work area 는 `GetMonitorInfoW` 직접 호출 대신 [DpiHelper.GetWorkArea](../Core/Dpi/DpiHelper.cs) (primary 모니터 폴백 내장) 를 재사용해 hMonitor 무효/조회 실패 시 rcWork 가 `(0,0,0,0)` 으로 떨어져 다이얼로그가 화면 좌상단 (0,0) 에 박히던 결함을 방지 (감사 High ③, 2026-06-01 — P4 폴백 로직 단일 구현 재사용). `RegisterStandardClass(className, wndProc, hbrBackground?)` is the **single entry point** for all `WNDCLASSEXW` registration — internally enforces `hCursor=IDC_ARROW` so no caller can omit the class cursor (defends against `IDC_APPSTARTING` cursor fallback on first-launch hover). `ShowFieldError(hwndOwner, hwndField, message, title)` 는 다이얼로그 EDIT 필드 검증 실패 시 공통 에러 표시 — 안내 `MessageBoxW`(자체 메시지 루프라 `ModalDialogLoop.RunExternal` 가드) + `SetFocus(hwndField)` + `SendMessageW(EM_SETSEL, 0, -1)` 전체 선택 3단을 단일화(감사 묶음 3 DUP-5, 현재 `ScaleInputDialog` 2 검증 경로 적용; EM_SETSEL 은 EDIT 전용이라 호출자가 EDIT 필드일 때만 사용, ScrollIntoView 같은 선행 동작은 호출자 책임) | `CalculateNonClientHeight/Width`, `CalculateFontHeightPx`, `ApplyFont`, `CreateDialogFont`, `CalculateDialogPosition`, `RegisterStandardClass`, `ShowFieldError` |
| [Core/Windowing/ScrollableDialogHelper](../Core/Windowing/ScrollableDialogHelper.cs) | 세로 스크롤 다이얼로그(`SettingsDialog` / `CleanupDialog`) 공용 스크롤 계산·WinAPI 호출. `SetupVScrollbar(hwndViewport, totalContentHeight, viewportClientHeight)` 는 초기 `SCROLLINFO`(nMin=0 / nMax=Max(0,total-1) / nPage=Max(1,viewport) / nPos=0, fMask=RANGE\|PAGE\|POS, redraw=true) 구성 + `SetScrollInfo` 적용 — 두 다이얼로그가 각자 복제하던 셋업 흡수(감사 묶음 3 DUP-2, nMax/nPage 음수·0 방어 클램프 동반). `ScrollTo(hwndViewport, ref scrollPos, scrollMax, newPos)` 는 `SIF_POS` 갱신 + `ScrollWindowEx(SW_SCROLLCHILDREN \| SW_INVALIDATE \| SW_ERASE)` 로 자식 일괄 이동, `ResolveVScrollPosition` 은 SB_* 코드 → 목표 위치 해석, `CalculateWheelScrollPos` 는 `delta / WHEEL_DELTA × WheelLineStep × lineHeight` 계산. 상태(`scrollPos` 등)는 호출자 소유 — `ref` 로 동기화. `WheelLineStep = 3` 상수 공유 | `SetupVScrollbar`, `ScrollTo`, `ResolveVScrollPosition`, `CalculateWheelScrollPos`, `WheelLineStep` |
...
| [App/UI/Animation.cs](../App/UI/Animation.cs) | Static facade over `OverlayAnimator`. **`internal BuildAnimationConfig(config)`** extracts primitives and composes the "애니메이션 사용" master gate for the **floating badge** — `ChangeHighlight`/`SlideAnimation` are AND-composed with `config.AnimationEnabled` so `animation_enabled` gates fade (engine-side) plus highlight·slide (PR-22; Core unchanged — the engine consumes the composed flags as-is; `internal` for `AnimationFacadeTests`). 커서 헤일로는 별도 파사드라 본 합성을 거치지 않고 [`CursorOverlay.SetImeState`](../App/UI/CursorOverlay.cs) 가 `AnimationEnabled && CursorChangeHighlight` 로 팝을 평행 게이팅 (PR-22 후속). **`TriggerShow(...) → bool`** — 실제로 표시했으면 `true`, `NonKorean` + `NonKoreanImeMode.Hide` 가드로 숨김 처리했거나 엔진이 없으면 `false` (bug-hunt 2026-08-02 G4). `Program._indicatorVisible` 은 **이 결과로만** 세운다 — 선-대입하면 Hide 가드 경로에서 화면과 어긋난 채 박제되고, 애니메이터가 이미 `Hidden` phase 면 `onHidden` 훅조차 발화하지 않아 되돌릴 기회가 없다 ([conventions.md](conventions.md) invariant: `_indicatorVisible = true` 0 매치). `SetDimMode` routes `NonKoreanImeMode.Dim && state == NonKorean` into a `bool`. 콜백 배선에 `onTrackPosition: Overlay.TrackPosition` 포함 (slide+highlight 합성 시 slide 가 위치만 추적, highlight 가 blit 전담) |
| [App/UI/CursorRenderer.cs](../App/UI/CursorRenderer.cs) | 커서 추종 인디의 distance-field 분석적 AA 픽셀 셰이더. `LayeredCursorBase` 의 `renderToDib` 콜백으로 주입되어 `CursorStyle` + `CursorMetrics` 를 받아 DIB BGRA32 픽셀에 직접 쓴다. 평상시(soft=0) 각 동심원 = 코어 (사용자 색상) + 헤일로 (흰색 × `HaloOpacity`). CAPS OFF 시 Outer skip. **PR-21** — 반지름·두께 ×`HighlightScale`. **PR-29/31** — `MotionSoftness`&gt;0 이면 하드 코어 없이 가우시안 안개(σ≈baseHalo×(1+soft×13), 색+흰색 혼합) + `RingAlpha*` 스케일; Outer는 CAPS ON 일 때만. stride = `DibSectionFactory.BytesPerPixel`; 안개 whiteness = `DefaultConfig.CursorFogWhiteness*` |
| [App/UI/CursorMotionDim.cs](../App/UI/CursorMotionDim.cs) | PR-29/31 커서 표시 **순수** 상태 전이. `AdvanceForMode`(Soft=항상 딤 / Sharp=항상 Full / Motion=enter·settle) · `EffectiveSoftness` · `RingAlphas`. 창 알파는 Full. 단위 `CursorMotionDimTests` |
| [App/UI/CursorOverlay.cs](../App/UI/CursorOverlay.cs) | `LayeredCursorBase` 위의 정적 파사드 — `Initialize` / `HandleConfigChanged` / `HandleCursorMotionTimer` / `HandleCursorPopTimer` / `SetImeState` / `SetCapsLock` / `Dispose`. **PR-21 IME 팝**. **PR-31 커서 표시** — AlwaysShow에서 `CursorDisplayMode`→`AdvanceForMode` → soft/α 배선; IME 팝 중 Soft/딤 안개 유지(`HighlightScale`만). **PR-32** 셸·메뉴 호버 숨김은 `OverlaySuppressProbe` · topmost 재적용 |
| [App/UI/Tray.LeftClick.cs](../App/UI/Tray.LeftClick.cs) | 트레이 좌클릭(`TrayClickAction.Toggle`)의 **순수** 상태 전이 + `IndicatorVisibility` enum(Both/BadgeOnly/CursorOnly/None). `GetVisibility(config)` 가 `UserHidden`+`CursorIndicatorEnabled` 조합을 단계로 읽고, `ComputeLeftClickCycle(config)` 가 **다음 단계로 순환**(둘 다→배지만→헤일로만→모두 숨김→둘 다). 조합이 곧 상태라 복원 기록을 저장하지 않는다. `GetShapes(visibility)` 는 트레이 아이콘에 그릴 도형(배지/헤일로) 판정 — 보이는 요소만 그려 네 상태가 시각적으로 구별된다. 부수효과 0 → 저장·오버레이 적용·아이콘 갱신은 `Program.HandleTrayToggle` 담당. 단위 `TrayLeftClickToggleTests` |
| [App/UI/Tray.cs](../App/UI/Tray.cs) + [Tray.Menu.cs](../App/UI/Tray.Menu.cs) | Static facade over `NotifyIconManager`. 라이프사이클 (Initialize/HandleAddRetryTimer/UpdateState/OnUpdateFound/Recreate/Remove — `Remove` 는 종료·재생성·`tray_enabled` 해제 **세 경로가 공유**하므로 `TrayRemoveReason` 으로 맥락을 명시받고(기본값 없음) 로그 문구·`NIM_DELETE` 실패 심각도를 그 값으로 가른다, 2026-08-03 B-2 후속. `UpdateState` 는 아이콘 생성 실패 시 NULL 핸들을 셸에 넘기지 않고 **직전 유효 아이콘을 유지**한 채 툴팁만 갱신한다, 렌더 엔진의 "이전 DIB 유지"와 같은 열화 방식 — **`Initialize` / `HandleAddRetryTimer` 도 같은 정책**이라 무효 `HICON` 은 등록하지 않고 재시도가 **매 회차 아이콘부터 다시 만든다**, bug-hunt 2026-08-02 G7) + WM_COMMAND 디스패치 + `_pendingUpdate` 저장 + helpers (CleanupPositions/SetDefaultPositionToCurrent/BuildTooltip/ApplyQuickOpacity). `Tray.Menu.cs` partial 는 `ShowMenu` 만 분리 (메뉴 빌더). PR-04 분해 — schtasks 는 [App/Startup/StartupTaskManager](../App/Startup/StartupTaskManager.cs), 위치 정리 비즈니스 로직은 `PositionCleanupService`, URL/파일 열기는 `UriLauncher` 로 위임. **셸 호출 재진입 가드** `_shellCallInProgress` (bug-hunt 2026-08-02 G8): `Shell_NotifyIconW` 는 explorer 로 가는 **블로킹 크로스프로세스 SendMessage** 라 그 대기 중 이 스레드로 *sent* 된 `WM_SETTINGCHANGE`/`WM_THEMECHANGED` 가 디스패치돼 `UpdateState` 가 재진입한다 → `_currentIcon` 의 「넘기고→해제하고→재대입」 세 걸음이 뒤엉켜 **셸이 그리고 있는 HICON 을 파괴**. 재진입은 버리지 않고(안쪽이 더 새 색을 들고 온다) `_updatePending` + `UpdateStateCore` 로 바깥 프레임 끝에서 소비하며, `HandleAddRetryTimer` 도 같은 가드를 공유. **`OpenConfigFile`**: 일반 = notepad; **elevated** = `explorer /select` (Medium IL — High IL notepad 전파 회피, 감사 M2). `IsCurrentProcessElevated` 호출처 = Menu OR 체크 + OpenConfig |
| [App/Startup/StartupTaskManager.cs](../App/Startup/StartupTaskManager.cs) | Windows Task Scheduler (`schtasks.exe`) 기반 "시작 시 자동 실행" 작업 등록/조회/동기화. UI 와 무관한 XML 조립 + CLI 호출 + 결과 검증 책임. 외부 의존성은 `Logger` + `XmlEntityCodec` 만 — 메뉴 ID 는 모르고 호출자(Tray) 가 ID 와 핸들러를 mapping. PR-04 분해 산물. **PR-15** — `BuildStartupTaskXml(exePath, adminElevation)` 시그니처 확장으로 `<RunLevel>` 을 `LeastPrivilege` (default) vs `HighestAvailable` (admin_elevation=true) 분기 + `SyncStartupPathAsync` 의 `expectedRunLevel` 도 config 에서 derive 해 admin 토글 시 자동 재등록 + 신규 `ReregisterIfAdminChanged(config)` 헬퍼 (Tray/SettingsDialog 토글 직후 호출). **파이프 데드락 방지**: `schtasks` 자식 프로세스의 stdout/stderr 를 `ReadToEndAsync` 로 **동시** 흡수한 뒤 `WaitForExit` — 한쪽만 읽고 다른 쪽 파이프 버퍼가 포화되면 자식이 write 에서 블록되고 부모는 EOF 대기로 맞물려 데드락하던 위험을 3개 호출처 (`QueryRegisteredTask` exit-code-only · `/query /xml` · 일반 `RunSchtasks`) 에서 제거. **타임아웃 처리** `KillTimedOutChild` (bug-hunt 2026-08-02 G12): 같은 3개 호출처가 `WaitForExit(timeout)` **반환값을 확인**하고 미종료 시 `Kill(entireProcessTree: true)` — 종전에는 조회 2경로가 반환값을 버리고 `ExitCode` 를 읽어 `InvalidOperationException` 이 catch 필터에서 **"미등록"으로 삼켜졌고**(트레이 「시작 프로그램」 체크가 실제와 반대 표시), 고아 자식 + 리다이렉트 파이프 핸들 2개가 우클릭마다 누적됐다 |
| [App/UI/TrayIcon.cs](../App/UI/TrayIcon.cs) | GDI-based dynamic icon bitmap — **헤일로 링 + 배지** 디자인(제품의 두 표시 요소 은유). IME 상태별 배경색·도형색을 `CreateIcon`이 즉석에서 그려 `SafeIconHandle`로 반환. **GDI 도형 API 대신 32bpp DIB 픽셀에 직접 렌더링**(`PaintIcon`) — GDI 는 AA 가 없어 16px 원이 계단처럼 거칠다. 링·둥근 모서리·배지 경계 픽셀만 4×4 서브샘플 커버리지로 보간. 구성: 둥근 모서리 배경(반지름 `0.22×side`, 바깥 투명) + 링(`min/2-1`, 두께 `min/8`) + 후광(폭 `min/9`, α 0.65, 감쇠 2.2, **32px 미만 생략**) + 배지(`iconW/3`×`iconH/5`, 모서리 `0.25×높이`). 숨김은 보이는 도형만 그려 4상태 구별(링+배지 / 배지 / 링 / 없음), 도형 선택은 `Tray.GetShapes`. 치수는 16px 실측. 앱 아이콘 `koenvue.ico` 도 동일 도형·비율 8해상도 |
| [App/Messaging/AppMessages.cs](../App/Messaging/AppMessages.cs) | `WM_APP + N` / timer ID constants for cross-thread signalling (UI 밖 — Detector/Config → UI 역의존 방지) |
...
| [App/UI/Dialogs/SettingsDialog.cs](../App/UI/Dialogs/SettingsDialog.cs) (+ `.Layout.cs` + `.Fields.cs` + `.Scroll.cs`) | Scrollable settings dialog (PR-21 14 sections / **75 필드** — 일반 / 플로팅 배지 / 커서 헤일로 3 대분류. 커서 = "동심원" 10 + **PR-31 표시 Combo + 안개 α/soft** + "전환 효과" 2; `cursor_change_highlight` 는 트레이 전용). `BuildChildren` 은 [`SettingsDialog.Layout.cs`](../App/UI/Dialogs/SettingsDialog.Layout.cs) `SettingsLayout` + `BuildLayout` 로 레이아웃 분리 (IMP-2) |
