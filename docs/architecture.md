...
| [App/UI/Tray.LeftClick.cs](../App/UI/Tray.LeftClick.cs) | 트레이 좌클릭(`TrayClickAction.Toggle`)의 **순수** 상태 전이 — `ComputeLeftClickToggle(config)` 가 플로팅 배지(`UserHidden`)와 커서 헤일로(`CursorIndicatorEnabled`)를 함께 다룬다. 하나라도 보이면 가시 상태를 스냅샷(`TrayHideRestore*`)에 기록하고 둘 다 숨김, 둘 다 숨김이면 스냅샷대로만 복원(원래 꺼둔 쪽은 유지). 부수효과 0 → 저장·오버레이 적용·아이콘 갱신은 `Program.HandleTrayToggle` 담당. `CountHiddenIndicators(config)` 는 취소선 줄 수(0/1/2) 판정 — **설정상 비활성은 세지 않는다**(헤일로는 `CursorIndicatorEnabled` 하나가 기능 끔·일시 숨김을 겸하므로 `TrayHideRestoreCursor` 기록이 있을 때만 카운트). 단위 `TrayLeftClickToggleTests` |
...
| [App/UI/TrayIcon.cs](../App/UI/TrayIcon.cs) | GDI-based dynamic icon bitmap — 캐럿+점(caret+dot) 디자인 고정. IME 상태별 배경색을 `CreateIcon`이 즉석에서 그려 `SafeIconHandle`로 반환. **숨김 표시는 취소선 개수** — 배지·헤일로 중 하나만 숨김=단일선(`iconH/4`), 둘 다 숨김=이중선(`iconH/6`·간격 `iconH/8`). 16px 에서 길이 차이는 판별이 어려워 개수 축을 택함 |
