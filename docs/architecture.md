...
| [App/UI/Animation.cs](../App/UI/Animation.cs) | Static facade over `OverlayAnimator`. `BuildAnimationConfig(config)` extracts primitives; `SetDimMode` routes `NonKoreanImeMode.Dim && state == NonKorean` into a `bool`. 콜백 배선에 `onTrackPosition: Overlay.TrackPosition` 포함 (slide+highlight 합성 시 slide 가 위치만 추적, highlight 가 blit 전담) |
