...
| Structs / constants | [Core/Native/Win32Types.cs](../Core/Native/Win32Types.cs) (`Win32Constants` class — SM/WS/DWMWA/`LOWORD_MASK`/`KEY_PRESSED`/etc.) |
| 32bpp DIB 픽셀당 바이트 (`BytesPerPixel`) | [Core/Windowing/DibSectionFactory](../Core/Windowing/DibSectionFactory.cs) (`public const int BytesPerPixel = 4` — DIB 생성처가 stride 단일 진실원, 두 렌더 엔진이 참조) |
...
- **비-numeric(색상/문자열/배열) 디폴트도 `DefaultConfig` 단일 진실원** (감사 묶음 2, DUP-1, 2026-06-02 — PR-17 numeric 축의 비-numeric 완성). 위 numeric init invariant 가 잡는 `AppConfig` 수치 디폴트와 동일 정신으로, 상태 색상 7개 (`DefaultHangulBg`=`#16A34A` 등 Bg/Fg 6 + `DefaultBorderColor`) · 인디케이터 폰트/라벨 4개 (`DefaultIndicatorFontFamily`=`맑은 고딕` + 라벨 3) · 오버레이 클래스명 (`DefaultOverlayClassName`=`KoEnVueOverlay`) const + 시스템 숨김 배열 2개 (`DefaultSystemHideClasses`/`DefaultSystemHideProcesses`) property 를 [DefaultConfig.cs](../App/Config/DefaultConfig.cs) 단일 진실원으로 추출. `AppConfig` 14 init 디폴트 + `Settings.EnsureSubObjects`/`ValidateAdvanced` 폴백이 모두 본 const/property 참조 — 이전엔 같은 리터럴이 양쪽에 흩어져 "양쪽 수기 유지" 주석에 의존했음 (한쪽만 바꾸면 silent 불일치). 배열은 property(`=>`) 라 호출마다 새 배열 반환 (`TrayQuickOpacityPresets` 패턴 — 공유 변형 위험 0). **위 numeric init `\}\s*=\s*리터럴\s*;` 가드는 string/array 디폴트를 의도적으로 제외(array `];` 제외 규칙)하므로** 비-numeric 축은 별도 회귀 차단 invariant 로 가드 — `git grep '"#16A34A"\|"맑은 고딕"\|"KoEnVueOverlay"\|"Progman"\|"ShellExperienceHost"' App/Models/AppConfig.cs App/Config/Settings.cs` → **0 매치** (색상/폰트/클래스명/숨김 클래스·프로세스 리터럴은 `DefaultConfig.cs` 만 정의 보유, 두 소비처는 const/property 참조만). PR-17 의 numeric init invariant 와 **형제 가드**.
- **"같은 의미의 const 가 다른 파일에 이미 존재하는데 인라인 매직 넘버가 잔존" 단일화** (감사 묶음 1, 2026-06-02). config 디폴트가 아닌 코드 상수도 같은 정신 — 이미 단일 진실원 const 가 있으면 인라인 리터럴 금지. 묶음 1 에서 통일한 자리: 32bpp DIB 픽셀당 바이트 `4` → [`DibSectionFactory.BytesPerPixel`](../Core/Windowing/DibSectionFactory.cs) (DIB 생성처가 stride 진실원, 두 렌더 엔진 `LayeredOverlayBase`·`LayeredCursorBase` 가 `w*h*BytesPerPixel`·`i*BytesPerPixel` 참조 — `CursorRenderer` 에 이미 있던 동명 const 의 Core 짝); `GetAsyncKeyState` 눌림 비트 `0x8000` → [`Win32Constants.KEY_PRESSED`](../Core/Native/Win32Types.cs) (`Program.IsDragModifierPressed` 의 지역 const + `LayeredOverlayBase` Shift 감지 인라인 단일화 — App·Core 공유라 Core 배치); WM_COMMAND/WM_VSCROLL `& 0xFFFF` → [`Win32Constants.LOWORD_MASK`](../Core/Native/Win32Types.cs) (다이얼로그 5곳 — `Program.cs` 만 쓰던 기존 const 로 통일); `points-per-inch 72` → `LayeredOverlayBase.PointsPerInch` (`Win32DialogHelper.PointsPerInch` 의 형제 — 공용화 대신 엔진 로컬 const 채택); premultiply 알파 최대 `255` → `byte.MaxValue` (곱셈·`a==255` 가드만; 나눗셈 `b*a/255` 류는 의미 보존 위해 리터럴 유지). HttpClientLite (`HttpStatusOk`/`MaxResponseBytes`)·UriLauncher (`ShellExecuteSuccessThreshold`) 등 모듈 로컬 단일 사용 const 는 해당 파일 안에 둠 (Win32 도메인 상수 아님). **보존 예외** — `WinHttp.cs` 미사용 const 4개 (감사 HC-17) 는 사용 중 상수의 짝/대안이라 제거하지 않음 (참고용 미사용 — dead 아님).
...
git grep "DllImport" -- '*.cs'       # banned, use [LibraryImport] — docs 노이즈 제외
...
git grep "GetTokenInformation"      Core/Native/   # PR-15: ≥1 presence (선언·호출·주석 — 카운트 drift OK, Advapi32 RID 추출)
git grep "RunLevelHighestAvailable" App/Startup/   # PR-15: 3 (const + BuildStartupTaskXml 사용 2)
git grep "RelaunchParentPidEnvVarName" App/Bootstrap/   # PR-15 후속 fix: 4 (const + 사용 3 — KOENVUE_RELAUNCH_PARENT_PID)
git grep "Win32Constants.MB_OK"        App/             # AUDIT 묶음 3 후: **3** (AdminElevation / SettingsDialog / Tray.ShowMessage). **핵심 invariant 는 카운트가 아니라** App/UI `MessageBoxW` 단일 OK 버튼이 100% named `Win32Constants.MB_OK` (positional uType=0 금지, 아래 두 줄) — 카운트는 헬퍼 추출마다 변하므로 정성 패턴이 단일 진실원
git grep -E "uType:\s*0\b"             App/             # PR-15 후속 fix #2: 0 (named argument 형태의 매직 넘버 0 잔존 없음)
git grep -nE "User32\.MessageBoxW\([^)]*,\s*0\s*\)" App/  # PR-15 후속 fix #2 cleanup: 0 — positional uType=0 금지
git grep "AdminElevationChangeNotice"  App/           # PR-15 후속 fix #3: 4 (I18n 3 + Tray 1)
git grep -E "ClearReentryGuard|SetRelaunchParentPidForTrayRestart" App/   # PR-15 후속 fix #3: 0
git grep "IsCurrentProcessElevated()" App/UI/   # 호출 site **2** (Tray.Menu.cs 메뉴 체크 OR + Tray.OpenConfigFile elevated→explorer 분기). 주석 매치 포함 시 텍스트 카운트는 더 클 수 있음 — **호출 카운트만** 게이트
git grep "MenuAdminElevationExternal" App/   # PR-15 후속 fix #5: 4 (I18n 3 + Tray.Menu 1)
...
git grep -n "Settings\.Save(" -- '*.cs'   # bug-hunt 확정 #28·#47: **1** (Program.SaveAndSync 단일 진입점). 저장 경로가 헬퍼를 우회하면 3-way 병합으로 들어온 값에 **전이 재적용이 통째로 빠진다** — 커서 헤일로·트레이·로거가 병합 전 값에 고정되고, Save 의 mtime self-bump 가 핫리로드까지 막아 자기치유도 없다
git grep -n "ApplyConfigTransition(" Program.cs   # bug-hunt 확정 #28·#47 + 3차 C: **4** (정의 1 + 호출 3 — HandleConfigChanged 핫리로드 · SaveAndSync 병합 · HandleMenuCommand 의 updateConfig 람다). 설정 교체를 앱 전역에 반영하는 경로는 이 셋뿐이어야 한다. **3차 C 로 늘었다** — 람다가 적용자를 자체 나열하던 동안 그 목록에서 Logger 만 빠져, 상세 설정에서 바꾼 로그 설정이 재시작 전까지 반영되지 않았다
git grep -nE "Logger\.(SetLevel|Initialize)\(" Program.cs   # bug-hunt 3차 C: **4** (부팅 2 + ApplyConfigTransition 2). 런타임 로그 설정 적용은 전이 함수 **안에만** 있어야 한다 — 호출자가 자체 나열로 되돌아가면 이 카운트는 그대로면서 그 경로만 조용히 빠진다
git grep -n "InvalidateSessionPositions(" Program.cs   # bug-hunt 3차 Q: **2** (정의 1 + ApplyConfigTransition 호출 1). 단위 테스트는 이 함수를 직접 부르므로 **호출되는지는 검증하지 못한다** — 그쪽은 이 grep 이 고정한다
git grep -n "Tray.RunShellCall\|private static void RunShellCall" App/UI/   # bug-hunt 3차 A·B: **1** (정의만 — 호출은 같은 파일 안이라 Tray. 접두가 붙지 않는다). 셸 호출 가드가 이 헬퍼 밖으로 흩어졌는지 보는 보조 신호
...
Verification: `git grep "DllImport" -- '*.cs'` must return 0.
...
- **`Win32DialogHelper.ShowFieldError(hwndOwner, hwndField, message, title)`** for EDIT-field validation failure — MessageBox(`RunExternal` 가드) + `SetFocus` + `SendMessage(EM_SETSEL,0,-1)` 단일화 (감사 묶음 3 DUP-5, 현재 `ScaleInputDialog` 만; `SettingsDialog` commit 에러는 ScrollIntoView 선행 동작 얽힘으로 보류). EDIT 전용 — 호출자가 EDIT 필드일 때만 사용
- **`ScrollableDialogHelper.SetupVScrollbar(hwndViewport, totalContentHeight, viewportClientHeight)`** for the initial vertical `SCROLLINFO` setup — **스크롤형 다이얼로그(`SettingsDialog` / `CleanupDialog`)만** (`ScaleInputDialog` 은 비스크롤). `ScrollTo`/`ResolveVScrollPosition`/`CalculateWheelScrollPos` 공유면에 셋업까지 합쳐 SCROLLINFO 복제 제거 (감사 묶음 3 DUP-2)
...
- **`dotnet test tests/KoEnVue.Tests/`** — **전부 통과**가 기준. 테스트·파일 개수는 적지 않는다 — 테스트가 늘 때마다 낡는다(이 자리에 남아 있던 옛 baseline 수치를 2026-09-17 제거, [release-procedure §1](release-procedure.md) 과 같은 원칙). 개수가 필요하면 실행 결과의 `통과:` 값이나 `ls tests/KoEnVue.Tests/Unit/*.cs | wc -l` 로 센다. ⚠️ **테스트 csproj/디렉토리 명시 필수** — repo 루트에서 인자 없이 `dotnet test` 하면 cwd 의 메인 `KoEnVue.csproj`(테스트 0개)를 잡아 **0개 실행 후 exit 0** 으로 통과처럼 보이는 착시. 반드시 `dotnet test tests/KoEnVue.Tests/` 또는 `tests\KoEnVue.Tests\KoEnVue.Tests.csproj` 로 실행:
  ...
  - **감사 ⑩ + 합성**: `OverlayAnimatorTests`
  - **PR-22**: `AnimationFacadeTests`
  - **IMP-2**: `SettingsLayoutTests`
