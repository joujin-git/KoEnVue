...
- **`dotnet test tests/KoEnVue.Tests/`** — 현재 baseline **90 PASS** (PR-10 40 + PR-20 25 + config 머지 P0 10 + 감사 ⑩ 3 + PR-22 4 + IMP-2 8). Unit/ 디렉토리 10 파일. ⚠️ **테스트 csproj/디렉토리 명시 필수** — repo 루트에서 인자 없이 `dotnet test` 하면 cwd 의 메인 `KoEnVue.csproj`(테스트 0개)를 잡아 **0개 실행 후 exit 0** 으로 통과처럼 보이는 착시. 반드시 `dotnet test tests/KoEnVue.Tests/` 또는 `tests\KoEnVue.Tests\KoEnVue.Tests.csproj` 로 실행:
