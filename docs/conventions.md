...
git grep "Win32Constants.MB_OK"        App/             # AUDIT 묶음 3 후: 4 (Tray.cs ×2 — IDM_ADMIN_ELEVATION 안내 + ShowMessage 헬퍼 / Dialogs/SettingsDialog.cs ×1 — 필드 commit 에러 / AdminElevation.cs ×1 — ShowDeniedMessage). 이전 7 에서, AUDIT 묶음 3 (DUP-5/6) 이 ScaleInputDialog ×2 를 Core `Win32DialogHelper.ShowFieldError` (Core/Windowing 1건) 로 + Tray 안내 2곳을 `ShowMessage` 헬퍼로 단일화 → 4. **핵심 invariant 는 카운트가 아니라** App/UI `MessageBoxW` 단일 OK 버튼이 100% named `Win32Constants.MB_OK` (positional uType=0 금지, 아래 두 줄) — 카운트는 헬퍼 추출마다 변하므로 정성 패턴이 단일 진실원
...
- **`dotnet test tests/KoEnVue.Tests/`** — 현재 baseline **100 PASS** (PR-10 40 + PR-20 25 + config 머지 P0 10 + 감사 ⑩ 3 + PR-22 4 + IMP-2 8 + PR-24 10). Unit/ 디렉토리 11 파일. ⚠️ **테스트 csproj/디렉토리 명시 필수** — repo 루트에서 인자 없이 `dotnet test` 하면 cwd 의 메인 `KoEnVue.csproj`(테스트 0개)를 잡아 **0개 실행 후 exit 0** 으로 통과처럼 보이는 착시. 반드시 `dotnet test tests/KoEnVue.Tests/` 또는 `tests\KoEnVue.Tests\KoEnVue.Tests.csproj` 로 실행:
  ...
  - **감사 ⑩ + 합성** (2026-06-02): `OverlayAnimatorTests` (`OverlayAnimator` slide+highlight 합성 3 PASS) — `TriggerShow` 에 IME 전환 + 위치 변경이 겹칠 때 slide 가 위치만 추적(`onTrackPosition`, blit 없이)하고 highlight 가 그 위치를 중심으로 그려, 두 트랙이 같은 윈도우의 위치/크기를 16ms 간격으로 다투지 않게 한 합성 동작을 박제 (`Slide_DuringHighlight_TracksPositionWithoutBlit` / `Slide_WithoutHighlight_BlitsPosition` / `Highlight_RunsRegardlessOfSlide`). FSM 전체가 아니라 **콜백 spy 로 관측 가능한 좁은 합성 규칙만** 단위로 떼어낸 사례 — `AnimationEnabled=false` 로 Hidden→Holding 동기 전이를 만들고 `onPositionOffset` / `onTrackPosition` / `onScaledSize` 콜백 호출 유무로 slide/highlight 활성을 판정 (GDI/실 WM_TIMER 펌프 없이). 나머지 FSM (페이드 보간, hold 만료, idle dim) 은 여전히 수동 smoke 영역 (line 아래 참조)
  ...
  - **IMP-2** (2026-06-02): `SettingsLayoutTests` (`SettingsDialog.BuildLayout` 파생 산술 8 PASS) — `BuildChildren` 분해(IMP-2) 시 가장 깨지기 쉬운 clamp/Max 파생식을 헤드리스로 박제 (LabelX = ContentPadInner / ControlX = +LabelColW+ColGap / ControlColW clamp 3케이스: 미발동/뷰포트 좁힘/`Max(1,…)` 바닥 / SectionContentW `Max(inner,…)` / 뷰포트 지오메트리 VpX/Y/W/H / 150% DPI 스케일은 logical 메트릭만 곱하고 호출자 주입 Pad 는 재스케일 안 함). `DialogShellContext` 를 HWND 없이 구성 — `BuildLayout` 이 순수 산술(HWND 무의존)이라 가능한 **순수 표면 단위 테스트** (`MergeWithDefaults`/`BuildAnimationConfig` 류, `BuildLayout` 가시성 `internal`). 행 y 누적·실제 GDI 렌더는 자동검증 불가라 수동 smoke 영역으로 남음
  - **PR-24** (2026-06-05): `SystemFilterMonitorScopeTests` (`SystemFilter.IsMonitorScopedShell` 분류 10 케이스) — 멀티모니터 cross-monitor 셸 무시 게이트가 쓰는 작업표시줄 분류를 박제 (작업표시줄 2종 + 대소문자 변형 → true / 바탕화면 `Progman`·`WorkerW` → false[전체 데스크톱 덮음, 게이트 제외] / 일반 UWP `ApplicationFrameWindow`·일반 앱·`ControlCenterWindow`·빈 문자열 → false). 분류가 깨지면 무시가 바탕화면까지 번져 보조 모니터 인디 미숨김 회귀(`Progman`→true) 또는 작업표시줄 미포착으로 PR-24 수정 무효(`Shell_TrayWnd`→false). `IsMonitorScopedShell` 는 `MatchesAny` 위임 순수 문자열 매칭이라 단위 테스트 가능 — `SameMonitor`(라이브 `MonitorFromWindow`) + `TryHandleFilter` 게이트(라이브 foreground)는 수동 smoke 영역. `MergeWithDefaults`/`BuildAnimationConfig` 류 순수 표면 단위 테스트 철학
