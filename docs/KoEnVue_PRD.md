...
- **투명도·크기·자석 스냅**은 모두 드래그·렌더링 동작에 영향을 주는 항목이라 상단에 묶어 배치. 자석 스냅은 토글형 체크 항목이며 기본은 켜짐
...
### 4.7 상세 설정 대화상자
- "상세 설정" 메뉴 클릭 시 스크롤 가능한 모달 대화상자가 열려, 트레이 메뉴로는 노출되지 않는 61개 설정 필드를 13개 섹션(표시 모드, 외관 — 크기·테두리·색상·투명도·텍스트·테마, 애니메이션, 감지 및 숨김, 앱별 프로필, 핫키, 트레이, 시스템, 다중 모니터, 고급)으로 묶어 테이블(`설명 | 입력 상자`) 형태로 노출
...
- 제외 항목: 트레이 메뉴로 이미 조작 가능한 항목(투명도, 크기 배율, 기본 위치, 시작 프로그램 등록, 자석 스냅, 위치 데이터, 트레이 활성화)과 구조가 복잡한 컬렉션 필드(앱 프로필 맵, 앱 필터 리스트, 시스템 숨김 클래스 목록, IME 폴백 체인), 내부 전용 필드(`config_version`, `overlay_class_name`)
