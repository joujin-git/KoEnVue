...
- **드래그 활성 키 (클릭 투과 게이트)** (`config.drag_modifier`, 트레이 "드래그 활성 키" 서브메뉴로 전환)
  - **`"none"`** (기본) — 기존 동작. `WM_NCHITTEST` 가 항상 `HTCAPTION` 을 반환해 오버레이가 모든 마우스 이벤트(좌클릭·우클릭·휠)를 소비. 0 픽셀 드래그도 클릭을 가로채므로 인디 아래 창의 버튼은 눌리지 않음
  - **`"ctrl"` / `"alt"` / `"ctrl_alt"`** — `WM_NCHITTEST` 가 메인 스레드에서 `GetAsyncKeyState` 로 모디파이어 상태를 확인해, 정확히 일치하는 조합이 눌려 있으면 `HTCAPTION`(드래그), 그 외에는 `HTTRANSPARENT`(아래 창으로 투과) 반환. `Ctrl` 모드는 `Ctrl ∧ ¬Alt` 엄격 판정이라 `Ctrl+Alt` 조합에 우발 트리거되지 않음. 드래그가 시작되면 `WM_ENTERSIZEMOVE` 모달 루프가 마우스 캡처를 잡으므로 도중에 모디파이어를 놓아도 드래그는 끊기지 않음
  - 용례: 인디를 창 종료 버튼·스크롤바·링크 위에 둔 채 그대로 조작하고 싶을 때. `Shift` 는 드래그 중 축 고정에 선점되어 있어 선택지에서 제외
...
- **Static 스타일**: `config.tray_icon_style = "static"` 이면 IME 상태를 트레이 아이콘 색으로 노출하지 않는다. 구현은 `TrayIcon.CreateIcon` 진입점에서 `state = ImeState.English` 로 단일 정규화 — IME 전환과 무관하게 항상 `english_bg` 로 렌더링된다 (CaretDot↔Static 전환 매번 재생성되는 경로와 호환)
...
### 4.7 상세 설정 대화상자
"상세 설정" 메뉴 클릭 시 스크롤 가능한 모달 대화상자가 열려, 트레이 메뉴로는 노출되지 않는 **62 개 설정 필드를 13 개 섹션**(① 표시 모드, ② 외관 — 크기·테두리, ③ 외관 — 색상·투명도, ④ 외관 — 텍스트, ⑤ 외관 — 테마, ⑥ 애니메이션, ⑦ 감지 및 숨김, ⑧ 앱별 프로필, ⑨ 핫키, ⑩ 트레이, ⑪ 시스템, ⑫ 다중 모니터, ⑬ 고급)으로 묶어 `설명 | 입력 상자` 2 컬럼 테이블 형태로 노출한다.
