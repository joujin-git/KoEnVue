...
- **짧은 좌클릭 = 일시 숨김 / 드래그 = 위치 이동**
  - 인디 위 짧은 좌클릭 → 일시 숨김(`_clickDismissed`). 오버레이가 사라져 그 자리 클릭이 아래 창으로 통과. 포커스 변경 또는 한/영(IME) 변경 시 재표시. 트레이「플로팅 배지 숨김」(`user_hidden`)과는 별개(영속·트레이 취소선 없음)
...
트레이 메뉴 "애니메이션 사용" 으로 전체 on/off, "플로팅 배지 변경 강조" / "커서 헤일로 변경 강조" 로 IME 전환 강조만 on/off. 기본은 모두 켜짐. 페이드 지속 시간·하이라이트 배율·슬라이드 속도는 "상세 설정" 에서 조정.
...
- **숨김 상태 시각 표시 = 취소선 개수**: 숨겨진 것이 있으면 캐럿+점 위에 수평 취소선을 중앙에 굵게 중첩 렌더링하며, **선 개수가 숨김 범위**를 나타낸다.
  - **단일선** — 플로팅 배지(`user_hidden`)와 커서 헤일로(`cursor_indicator_enabled`) 중 **하나만** 숨김. 두께 `iconH / 4` (16 px 에서 4 px, 20 px 고DPI 에서 5 px)
  - **이중선** — **둘 다** 숨김(트레이 좌클릭 일괄 숨김의 결과). 각 선 `iconH / 6`·간격 `iconH / 8`(최소 2 px)로 얇게 잡아 두 줄 + 간격 블록을 세로 중앙에 맞춘다
  - 아무것도 숨기지 않았으면 선 없음. 색상은 캐럿+점과 동일한 상태별 Fg(`HangulFg` / `EnglishFg` / `NonKoreanFg`) 공유
  - 길이(짧은 선/긴 선) 대신 개수를 택한 이유: 16 px 에서 길이 차이는 판별이 어렵다
  - **설정상 비활성은 세지 않는다** — 판정은 `Tray.CountHiddenIndicators` 단일 진실원. 커서 헤일로는 `cursor_indicator_enabled` 하나가 설정 창 「커서 헤일로 사용」 체크박스·트레이 「커서 헤일로 숨김」 토글·좌클릭 대상을 겸하므로, 값이 `false` 인 것만으로는 **원래 안 쓰는 기능**인지 **방금 숨긴 것**인지 알 수 없다. 그래서 좌클릭 숨김 진입 시점에만 갱신되는 `tray_hide_restore_cursor` 기록을 함께 보아, **그 기록이 있을 때만** 숨김으로 센다. 헤일로를 평소 꺼두고 쓰는 사용자에게 취소선이 상시 표시되지 않는다
  - 배지는 대칭이 아니다 — 배지에는 "기능 끄기" 설정이 없고 `user_hidden` 이 언제나 명시적 숨김을 뜻하므로 그대로 센다
...
| 값 | 동작 |
|----|------|
| `"toggle"` (기본) | **플로팅 배지 + 커서 헤일로 일괄 숨김/복원** (`config.user_hidden` + `config.cursor_indicator_enabled` 동시 제어) |
| `"settings"` | `config.json` 을 메모장(`notepad.exe`)으로 연다 |
| `"none"` | 좌클릭 무시 |

**토글 동작 세부 (`"toggle"`)** — 두 표시 요소를 **함께** 다룬다. 한쪽이라도 보이는 중이면 그 시점의 가시 상태를 복원 스냅샷(`tray_hide_restore_badge` / `tray_hide_restore_cursor`)에 기록하고 **둘 다 숨긴다**. 둘 다 숨겨진 상태에서 다시 누르면 **그 스냅샷대로만 되살린다** — 원래 꺼둬 있던 쪽은 계속 꺼진 채로 둔다. 그래서 배지만 쓰고 헤일로는 꺼둔 사용자가 좌클릭을 왕복해도 헤일로가 임의로 켜지지 않는다. 스냅샷이 비어 있으면(직접 편집 등) 좌클릭이 먹통이 되지 않도록 둘 다 되살린다.

상태 전이 계산은 `Tray.ComputeLeftClickToggle` 이 단일 진실원이며 부수효과가 없어 단위 테스트로 고정돼 있다. 계산 결과는 `config.json` 에 즉시 영구 저장되므로 앱 재기동·포그라운드 전환·세션 종료에도 유지된다. `user_hidden = true` 일 때는 감지 스레드의 IME/포커스/포그라운드 이벤트 5종이 인디를 다시 띄우지 않는다 — 사용자가 명시적으로 숨겼다는 의도를 모든 자동 복원 경로보다 우선 적용. 커서 헤일로 쪽은 `ApplyCursorConfigChange` 가 오버레이 윈도우를 생성/파괴한다.

**트레이 메뉴 반영** — 우클릭 메뉴의 「플로팅 배지 숨김」·「커서 헤일로 숨김」 체크 상태는 메뉴가 열릴 때마다 현재 config 로 구성되므로 좌클릭 직후 즉시 반영된다. 반대로 그 개별 메뉴 토글은 복원 스냅샷을 건드리지 않는다 — 스냅샷은 좌클릭 숨김 진입 시점에만 갱신되므로 다음 좌클릭이 항상 직전 가시 상태를 본다.
