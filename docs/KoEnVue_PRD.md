...
- **짧은 좌클릭 = 일시 숨김 / 드래그 = 위치 이동**
  - 인디 위 짧은 좌클릭 → 일시 숨김(`_clickDismissed`). 오버레이가 사라져 그 자리 클릭이 아래 창으로 통과. 포커스 변경 또는 한/영(IME) 변경 시 재표시. 트레이「플로팅 배지 숨김」(`user_hidden`)과는 별개 — 영속되지 않고 트레이 아이콘 도형도 그대로다(배지가 사라지지 않음)
...
### 4.1 아이콘
- **헤일로 링 + 배지 디자인** — 제품의 두 표시 요소를 그대로 은유한다. 바깥에 커서 헤일로를 뜻하는 링, 그 안에 플로팅 배지를 뜻하는 가로 사각형. IME 상태별로 **배경색과 도형색이 함께 바뀐다** (한글 `#16A34A` / 영문 `#D97706` / 비한국어 `#6B7280`, 도형은 각 상태의 Fg)
