...
- **제외 항목**: 트레이 메뉴로 이미 조작 가능한 항목(투명도, 크기 배율, 기본 위치, 시작 프로그램 등록, 자석 스냅, 애니메이션 사용, 변경 시 강조, 위치 데이터, 트레이 활성화)과 구조가 복잡한 컬렉션 필드(앱 프로필 맵, 앱 필터 리스트, 시스템 숨김 클래스 목록), 내부 전용 필드(`overlay_class_name`)
