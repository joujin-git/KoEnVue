...
- **색상 변환** → `ColorHelper.cs` 1곳에서만 구현. HEX→COLORREF 변환을 Overlay, TrayIcon이 공유
...
| 로깅 | System.Diagnostics.Trace 또는 자체 경량 로거 | 외부 로깅 프레임워크 불필요 |
...
| `System.Diagnostics` | .NET 기본 라이브러리 | 로깅 (Trace/Debug) | NuGet 불필요 |
| `System.IO` | .NET 기본 라이브러리 | 파일/경로 처리 | NuGet 불필요 |
