...
### 4.5 위치 기록 정리
- `indicator_positions` 전체 항목을 체크박스 다이얼로그로 표시하여 선택 삭제
...
| 인디 위치 | `indicator_positions` (프로세스명별 영구), `default_indicator_position` (저장 없는 앱의 기본 위치: `{corner, delta_x, delta_y}`), `snap_to_windows`, `snap_gap_px` |
