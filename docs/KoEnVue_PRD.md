...
- **제외 항목**
  ...
  - **`config.json` 직접 편집 전용 컬렉션 필드** (향후 전용 UI 도입 예정): `app_profiles` (앱 프로필 맵 — Phase 06 예정), `app_filter_mode` + `app_filter_list` (앱 화이트/블랙리스트), `system_hide_classes_user` / `system_hide_processes_user` (시스템 숨김 목록 사용자 확장)
...
| 트레이 | `tray_enabled`, `tray_tooltip`, `tray_click_action`, `tray_quick_opacity_presets` |
