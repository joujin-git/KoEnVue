...
- **기본 위치 (저장 안 된 앱)**: 위치 모드에 따라 별도 설정
  - **고정 모드**: `config.default_indicator_position` — Corner anchor + delta 를 포그라운드 앱이 있는 모니터의 작업 영역에 적용. 미설정 시 작업 영역 우상단 (`workArea.Right - 200, workArea.Top + 10`) 으로 폴백
  - **창 기준 모드**: `config.default_indicator_position_relative` — Corner anchor + delta 를 포그라운드 창의 DWM 프레임에 적용. 미설정 시 창 우상단 (`TopRight, -50, +10`) 으로 폴백
  - 모서리 anchor 방식이라 멀티 모니터·해상도 변경에 안정적이다(절대 좌표가 아니므로 해상도가 달라져도 의도한 모서리 근처에 유지)
...
- **드래그 활성 키 (드래그 개시 게이트)** (`config.drag_modifier`, 트레이 "드래그 활성 키" 서브메뉴로 전환)
  - **`"none"`** (기본) — `WM_NCHITTEST` 가 항상 `HTCAPTION` 을 반환해 오버레이 위 모든 좌클릭이 드래그를 개시. 0 픽셀 드래그도 클릭을 가로채므로 인디 아래 창의 버튼은 눌리지 않음
  - **`"ctrl"` / `"alt"` / `"ctrl_alt"`** — `WM_NCHITTEST` 가 클릭 시점에 `GetAsyncKeyState(VK_CONTROL / VK_MENU)` 를 직접 읽어, 모디파이어가 정확히 일치하는 조합으로 눌려 있으면 `HTCAPTION` (네이티브 드래그 시작), 아니면 `HTCLIENT` 를 반환해 클릭을 오버레이가 받지만 `WM_LBUTTONDOWN` 핸들러가 없어 무반응. `Ctrl` 모드는 `Ctrl ∧ ¬Alt` 엄격 판정이라 `Ctrl+Alt` 조합에 우발 트리거되지 않음. 드래그가 시작되면 `WM_ENTERSIZEMOVE` 모달 루프가 마우스 캡처를 잡으므로 도중에 모디파이어를 놓아도 드래그는 끊기지 않음
  - **크로스 프로세스 클릭 투과는 미지원** — 레이어드 윈도우의 반투명 칩 배경이 알파 > 0 이라 OS 가 오버레이 전 영역을 히트테스트 대상으로 취급하고, `HTTRANSPARENT` 반환도 MS 문서상 "같은 스레드 내 창" 에만 포워딩되므로 다른 프로세스 창으로 클릭이 전달되지 않는다. 투과를 지원하려면 `WS_EX_TRANSPARENT` 확장 스타일을 동적으로 켜고 꺼야 하는데, 이를 위한 타이머 폴링(30 Hz 웨이크업) 또는 저수준 키보드 훅(`[UnmanagedCallersOnly]` 경로 리스크, 300 ms 타임아웃 누적 시 OS 가 훅 비활성화) 의 런타임 비용·복잡도 대비 이득이 작아 도입 보류
  - 타이머·훅 없이 히트테스트 시점에만 `GetAsyncKeyState` 를 읽는 이유: 클릭당 마이크로초 수준 비용, 정상 상태에서 주기적 웨이크업 0, 키 상태와 클릭 사이에 race 불가. 유휴 비용이 0 인 최소 구현
  - 용례: 실수로 드래그 개시되는 것을 방지하고 의도적으로 모디파이어 + 클릭일 때만 이동시키고 싶을 때. `Shift` 는 드래그 중 축 고정에 선점되어 있어 선택지에서 제외

- **창 엣지 자석 스냅**: `config.snap_to_windows` (기본 켜짐, 트레이 메뉴 "창에 자석처럼 붙이기" 로 토글). 드래그 중 다른 최상위 창의 시각 프레임 엣지와 현재 모니터 작업 영역 엣지에 임계값(기본 10 px, DPI 스케일) 이내로 접근하면 자석처럼 스냅. `config.snap_gap_px` (기본 2 px, DPI 스케일)만큼 창 경계선과 간격을 두어 겹침을 방지하며, 화면(work area) 엣지에는 간격 없이 밀착. 드래그 시작 시 창 목록을 한 번 캐싱하며, 최소 크기 80 px 미만 창, 숨김·최소화 창, DWM cloaked 창(UWP 유령 창)은 후보에서 제외. 후보 rect 는 `DWMWA_EXTENDED_FRAME_BOUNDS` 로 얻은 시각 프레임을 사용해 비가시 리사이즈 테두리를 배제. Shift 축 잠금과 공존 — 잠긴 축은 스냅 대상에서 제외
...
- 현재 실행 중인 프로세스에는 "(실행 중)" / "(running)" 접미사 표시
- 전체 선택/해제 토글 지원
...
### 4.7 상세 설정 대화상자
"상세 설정" 메뉴 클릭 시 스크롤 가능한 모달 대화상자가 열려, 트레이 메뉴로는 노출되지 않는 설정 필드를 **12 개 섹션**(① 표시 모드, ② 외관 — 크기·테두리, ③ 외관 — 색상·투명도, ④ 외관 — 텍스트, ⑤ 외관 — 테마, ⑥ 애니메이션, ⑦ 감지 및 숨김, ⑧ 앱별 프로필, ⑨ 트레이, ⑩ 시스템, ⑪ 다중 모니터, ⑫ 고급)으로 묶어 `설명 | 입력 상자` 2 컬럼 테이블 형태로 노출한다.
...
| 트레이 | `tray_enabled`, `tray_tooltip`, `tray_click_action`, `tray_show_notification`, `tray_quick_opacity_presets` |
...
| 인디 위치 | `position_mode` (`fixed` / `window`), `indicator_positions` (고정 모드 프로세스명별 영구), `indicator_positions_relative` (창 기준 모드 프로세스명별 영구), `default_indicator_position` (고정 모드 기본 위치), `default_indicator_position_relative` (창 기준 모드 기본 위치), `snap_to_windows`, `snap_gap_px` |
