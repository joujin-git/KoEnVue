...
- **위치 모드** (`config.position_mode`, 트레이 메뉴 "한/영 배지 ▸ 위치 기준" 서브메뉴로 전환)
...
- **짧은 좌클릭 = 일시 숨김 / 드래그 = 위치 이동**
  - 인디 위 짧은 좌클릭 → 일시 숨김(`_clickDismissed`). 오버레이가 사라져 그 자리 클릭이 아래 창으로 통과. 포커스 변경 또는 한/영(IME) 변경 시 재표시. 트레이「한/영 배지 숨김」(`user_hidden`)과는 별개 — 영속되지 않고 트레이 아이콘 도형도 그대로다(배지가 사라지지 않음)
...
트레이 메뉴 "애니메이션 사용" 으로 전체 on/off, "한/영 배지 ▸ 전환 강조" / "커서 헤일로 ▸ 전환 강조" 로 IME 전환 강조만 on/off. 기본은 모두 켜짐. 페이드 지속 시간·하이라이트 배율·슬라이드 속도는 "상세 설정" 에서 조정.
...
### 4.1 아이콘
- **헤일로 링 + 배지 디자인** — 제품의 두 표시 요소를 그대로 은유한다. 바깥에 커서 헤일로를 뜻하는 링, 그 안에 한/영 배지를 뜻하는 가로 사각형. IME 상태별로 **배경색과 도형색이 함께 바뀐다** (한글 `#16A34A` / 영문 `#D97706` / 비한국어 `#6B7280`, 도형은 각 상태의 Fg)
...
- **숨김 상태 시각 표시 = 보이는 요소만 그리기**: 좌클릭 순환 4단계를 도형의 **있고 없음**으로 표현한다 — 링+배지 / 배지 / 링 / (도형 없음). 취소선으로 덧그리지 않는 이유는 도형이 넓어 같은 Fg 색 줄은 묻히고, 배경색으로 파내면 도형이 조각나 읽기 어렵기 때문. 16 px 에서는 "있고 없음" 이 가장 빨리 읽힌다.
  - **배경** — 둥근 모서리 정사각형(반지름 `min(W,H) * 0.22`), 모서리 바깥은 완전 투명. 앱 아이콘과 같은 인상을 준다
  - **링** — 바깥 반지름 `min(W,H)/2 - 1`, 두께 `min(W,H)/8`(최소 2 px). 가장자리 여백 1 px 를 두지 않으면(inset 0) 사각형 모서리에서 원이 평평하게 잘린다 — 16 px 실측
  - **배지** — 폭 `iconW/3`, 높이 `iconH/5`(최소 3 px), 모서리 반지름은 높이의 0.25 배(살짝만). 링 안쪽 중앙. 폭을 `iconW*3/8` 로 잡으면 16 px 에서 링 안쪽에 닿아 답답해진다 — 실측으로 축소
  - **후광(글로우)** — 링 바깥으로 폭 `min(W,H)/9` 만큼 번지며, 링에 접한 쪽이 최대 알파 0.65 이고 바깥으로 2.2 거듭제곱으로 옅어진다. **`min(W,H) < 32` 이면 생략** — 16 px 에서 후광은 링 경계를 뿌옇게 흐려 오히려 선명도를 떨어뜨린다(실측). 해상도별로 디테일을 달리하는 것은 아이콘 디자인의 통상 관행이며 `koenvue.ico` 도 같은 기준을 쓴다
  - **렌더링은 GDI 도형 API 가 아니라 32bpp DIB 픽셀에 직접** — GDI 는 안티앨리어싱을 하지 않아 16 px 원 둘레가 계단처럼 거칠어진다. 링·둥근 모서리·배지 모서리의 경계 픽셀만 4×4 서브샘플로 커버리지를 재어 배경↔전경을 보간한다(경계에서 떨어진 픽셀은 즉시 0/1 판정)
  - **모두 숨김 단계에서는 배경색만 남는다.** 도형이 사라져도 IME 상태는 그 배경색으로 계속 읽히므로 "앱은 살아 있고 표시만 전부 껐다" 가 드러난다
  - 어느 도형을 그릴지는 `Tray.GetShapes(visibility)` 단일 진실원 — 단위 테스트로 네 단계가 서로 다름 + 그리는 도형이 실제 표시 상태와 일치함을 고정
- **탐색기 앱 아이콘(`koenvue.ico`)도 같은 도형·같은 비율** — 16/20/24/32/48/64/128/256 px 8종을 PNG 임베드로 담는다. 트레이와 달리 IME 상태가 없으므로 한글 색(초록) 고정이고, 항상 "둘 다 보임" 단계로 그린다. 후광 생략 기준(32 px 미만)은 트레이와 동일
...
| 값 | 동작 |
|----|------|
| `"toggle"` (기본) | **표시 상태 4단계 순환** — 한/영 배지·커서 헤일로의 보임/숨김 조합을 한 칸씩 넘긴다 |
...
**트레이 메뉴 반영** — 우클릭 메뉴의 「한/영 배지 숨김」·「커서 헤일로 숨김」 체크 상태는 메뉴가 열릴 때마다 현재 config 로 구성되므로 좌클릭 직후 즉시 반영된다.
...
**`user_hidden` 리셋 경로** — `"toggle"` 이 아닌 값을 쓰는 환경에서도 dead-end 에 빠지지 않도록 트레이 우클릭 메뉴에 체크 토글 항목 **"한/영 배지 숨김"** 을 **최상위 맨 위**(대분류 서브메뉴 밖)에 배치한다 (4.2 참조) — 이 dead-end 방지 역할 때문에 접지 않는다. 그 외 리셋 경로로 `koenvue_config.json` 삭제 시 STJ 의 기본 unmapped-member handling 이 `user_hidden = false` 를 자연 복원하며, 필드를 수동 편집해도 hot reload + 다음 IME/포커스 이벤트로 표시가 돌아온다.
...
- **블록 순서**: 숨김 토글(한/영 배지·커서 헤일로) → 대분류 2종 → 공통(애니메이션 마스터) → 앱(시작·관리자) → 상세설정 → 종료. 설정 항목은 대상별 서브메뉴로 접어 최상위를 짧게 유지하되, **숨김 토글만은 접지 않고 맨 위에 남긴다** — 가장 자주 쓰는 항목이자 좌클릭 순환을 끈 환경의 유일한 해제 경로이기 때문. 대분류 안에서는 부모가 대상을 밝히므로 자식 라벨에 접두를 두지 않는다("한/영 배지 ▸ 전환 강조"). 자석 스냅·애니메이션·전환 강조는 기본 켜짐
- **한/영 배지 숨김**은 `config.user_hidden` 체크 토글 — 좌클릭 동작을 `"settings"`/`"none"` 으로 바꾸어 좌클릭 토글 경로가 막힌 경우에도 우클릭 메뉴로 항상 숨김 해제가 가능하도록 4.1a 의 dead-end 방지용. 내부적으로 `HandleTrayToggle` 과 동일한 `ApplyUserHiddenTransition` 헬퍼로 오버레이/트레이 아이콘 상태를 즉시 동기화하며, 토글 즉시 `Settings.Save` 로 koenvue_config.json 에 영구 저장
...
- **제외 항목**
  - 트레이 메뉴로 이미 조작 가능한 항목: 투명도, 크기 배율, 기본 위치, 시작 프로그램 등록, 자석 스냅, 애니메이션 사용, 한/영 배지·커서 헤일로 전환 강조, 위치 데이터, 위치 기준, 드래그 활성 키
