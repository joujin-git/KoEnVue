...
- **숨김 상태 시각 표시 = 보이는 요소만 그리기**: 좌클릭 순환 4단계를 도형의 **있고 없음**으로 표현한다 — 링+배지 / 배지 / 링 / (도형 없음). 취소선으로 덧그리지 않는 이유는 도형이 넓어 같은 Fg 색 줄은 묻히고, 배경색으로 파내면 도형이 조각나 읽기 어렵기 때문. 16 px 에서는 "있고 없음" 이 가장 빨리 읽힌다.
  - **링** — 바깥 반지름 `min(W,H)/2 - 1`, 두께 `min(W,H)/8`(최소 2 px). Fg 원을 채운 뒤 안쪽을 배경색 원으로 파내 만든다. 가장자리 여백 1 px 를 두지 않으면(inset 0) 사각형 모서리에서 원이 평평하게 잘린다 — 16 px 실측
  - **배지** — 폭 `iconW/3`, 높이 `iconH/5`(최소 3 px), 링 안쪽 중앙. 폭을 `iconW*3/8` 로 잡으면 16 px 에서 링 안쪽에 닿아 답답해진다 — 실측으로 축소
...
- **탐색기 앱 아이콘(`koenvue.ico`)도 같은 도형** — 16/20/24/32/48/64/128/256 px 8종을 PNG 임베드로 담는다. 트레이와 달리 IME 상태가 없으므로 한글 색(초록) 고정이며, 32 px 이상만 안티앨리어싱을 적용한다(작은 크기에서 AA 는 도형을 흐리게만 만든다)
