...
- **크기 예산**: Stage 5 검토 청소(catch 좁히기 + IID 상수 추출 + `partial class` 분할) 후 퍼블리시 exe = **4,903,424 bytes** (Stage 4 기준 4,911,104 bytes 대비 -7,680 bytes, -7.5 KB — 중복 코드 메타데이터 제거로 ILC 출력이 미세 감소). 누적적으로는 Stage 2 기준 4,891,136 bytes 대비 +12,288 bytes, +12 KB — 여전히 게이트 +100 KB 이내. ILC 트리밍이 엔진 인스턴스 패턴·델리게이트 콜백·제네릭 `JsonTypeInfo<T>` 주입을 모두 수용한다
