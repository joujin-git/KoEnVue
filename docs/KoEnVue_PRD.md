...
- **숨김 상태 시각 표시 = 취소선의 개수 + 위치**: 캐럿+점 위에 수평 취소선을 Fg 색으로 중첩 렌더링한다. **윗줄·아랫줄 두 자리를 먼저 고정**하고 단계에 따라 채우는 줄만 달리해, 순환 4단계가 **모두 시각적으로 구별**된다 — 선 없음 / 윗줄 / 아랫줄 / 둘 다 (4.1a 표).
  - **두께는 단일·이중 구분 없이 동일** — `iconH / 5`(최소 3 px). 16 px 에서 3 px, 20 px 고DPI 에서 4 px
  - **줄 간격** `iconH / 8`(최소 2 px). 두 줄 + 간격을 한 블록으로 보고 세로 중앙 정렬 — 16 px 에서 블록 8 px(윗줄 y=4\~6 · 아랫줄 y=9\~11 · 상하 여백 4 px), 20 px 에서 블록 10 px
  - 단일선도 이 블록의 윗줄·아랫줄 좌표를 그대로 쓴다. 단계가 바뀌어도 선이 같은 자리에 머물러 **위치 비교로 상태를 읽을 수 있다**
  - 색상은 캐럿+점과 동일한 상태별 Fg(`HangulFg` / `EnglishFg` / `NonKoreanFg`) 공유
  - 선 길이(짧은 선/긴 선)로 구분하지 않는 이유: 16 px 에서 길이 차이는 판별이 어렵다
  - 어느 줄을 채울지는 `Tray.GetStrikeLines(visibility)` 단일 진실원 — 배지만 보임 → 윗줄, 헤일로만 보임 → 아랫줄, 모두 숨김 → 둘 다. 단위 테스트로 네 단계가 서로 다름을 고정
...
| 1. 둘 다 보임 | 보임 | 보임 | 선 없음 |
| 2. 배지만 | 보임 | 숨김 | 취소선 **윗줄** |
| 3. 헤일로만 | 숨김 | 보임 | 취소선 **아랫줄** |
| 4. 모두 숨김 | 숨김 | 숨김 | 취소선 **윗줄 + 아랫줄** |
