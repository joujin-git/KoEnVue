...
### 4.7 상세 설정 대화상자
"상세 설정" 메뉴 클릭 시 스크롤 가능한 모달 대화상자가 열려, 트레이 메뉴로는 노출되지 않는 **61 개 설정 필드를 13 개 섹션**(① 표시 모드, ② 외관 — 크기·테두리, ③ 외관 — 색상·투명도, ④ 외관 — 텍스트, ⑤ 외관 — 테마, ⑥ 애니메이션, ⑦ 감지 및 숨김, ⑧ 앱별 프로필, ⑨ 핫키, ⑩ 트레이, ⑪ 시스템, ⑫ 다중 모니터, ⑬ 고급)으로 묶어 `설명 | 입력 상자` 2 컬럼 테이블 형태로 노출한다.
