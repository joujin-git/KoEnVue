...
- **로컬 [dotnet build](../KoEnVue.csproj) 통과** + **[dotnet test](../tests/KoEnVue.Tests/) 40/40 통과** + **[GitHub Actions](../.github/workflows/build.yml) 녹색** (이전 main 푸시 기준).
...
형식: `major.minor.build[.revision]` (2~4-part). `System.Version.TryParse` 가 받아들이는 모든 형태 OK. 빌드 시점에 [Directory.Build.targets](../Directory.Build.targets) 의 `GenerateVersionConstants` Target 가 `obj/.../Version.g.cs` 로 `DefaultConfig.AppVersion` partial 조각을 자동 생성하고, PE 헤더의 `AssemblyVersion` / `FileVersion` / `InformationalVersion` 3종도 같은 값에서 derive 합니다 (.NET SDK 기본 동작).
...
```bash
git tag vX.Y.Z          # 예: git tag v0.10.0
...
- **Choose a tag**: `vX.Y.Z` (방금 푸시한 태그). 접두 `v` 필수 — [UpdateChecker.NormalizeVersion](../App/Update/UpdateChecker.cs) 가 벗겨냅니다.
- **Release title**: `KoEnVue vX.Y.Z` 또는 자유 텍스트.
