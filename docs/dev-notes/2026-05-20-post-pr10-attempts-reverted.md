...
| # | 증상 | 시도 결과 |
|---|------|----------|
| 1 | MoveMode 진입 시 플로팅 배지 안 보임 (트레이 우클릭 → "인디 위치 이동" 메뉴 클릭) | 부분 개선 후 간헐적 재발 — 미해결 |
| 2 | 부팅 직후 플로팅 배지 잠깐 보였다 사라짐 | 미해결 (→ 2026-06-02 디바운스로 해결, 아래 후기) |
| 3 | "커서 헤일로 보임" 토글 ON 상태에서 부팅 시 플로팅 배지 안 보였다가 다른 앱 클릭하면 보임 | 미해결 (#2 와 동일 회귀로 추정) |
...
| 4 | 도넛 색상 / 투명도가 플로팅 배지 IdleOpacity 와 불일치 | 변경 적용 (revert) |
...
`ShowWindow` 미호출. 플로팅 배지는 `Animation.TriggerShow` 의 `wasHidden` 분기가 `SW_SHOW`
...
**결과**: 커서 도넛 자체는 시각 확인 가능해졌으나 부팅 직후 플로팅 배지 안 보이는 새 회귀
...
### 가설 C — 부팅 시 커서 도넛 z-order 가 플로팅 배지 가림 (회귀 #2, #3)

**전제**: 두 오버레이 모두 `WS_EX_TOPMOST`. 마지막에 `SW_SHOW` 된 윈도우가 TOPMOST
z-밴드 안에서 위. 커서 도넛 `SW_SHOWNOACTIVATE` 가 플로팅 배지 위로 올라옴.

**변경**: `App/UI/CursorOverlay.cs:ShowInternal` 끝에 `Overlay.ForceTopmost()` 호출 추가
→ 플로팅 배지를 z-order 최상으로 복귀.

**결과**: **가설 자체가 틀림** — 사용자 확인: "마우스 커서와 플로팅 배지 근처에 있지 않음".
로그 검증: 커서 위치 `(-1305, 911)`, 플로팅 배지 `(-220, 1302)`, 다른 모니터, 겹침 없음.
ForceTopmost 호출 제거.
...
| 파일 | 변경 |
|------|------|
| `App/UI/CursorOverlay.cs:BuildStyle` | `baseAlpha` 계산을 `config.CursorAlpha == 0.85` 디폴트 시 `config.IdleOpacity` 사용하도록 변경 — 플로팅 배지 dim-idle 알파와 일치 |
