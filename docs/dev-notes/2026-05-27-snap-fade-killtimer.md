# 부팅 플로팅 배지 깜박임 — SnapToTargetAlpha Fade KillTimer (2026-05-27)
...
부팅 직후 플로팅 배지가 잠깐 보였다가 사라지는 회귀 (이전 명명 회귀 #2/#3). 진단 로그:
...
1. KoEnVue 실행 중 종료 → 즉시 재부팅 → 플로팅 배지가 한 번에 페이드 인 (중간 alpha 떨어짐 0 회)
2. 위 1을 ×3 회 반복
