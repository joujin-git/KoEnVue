...
`AnimationFrameMs = 16` / `CursorAlwaysPollMs = 16` 에 달린 **"~60fps" 주석은 사실과 다르다.**
...
| 상수 | 위치 | 선언값 | 실제 배달 | 주석의 주장 |
|---|---|---|---|---|
| `CursorAlwaysPollMs` | [`App/Config/DefaultConfig.cs:331`](../../App/Config/DefaultConfig.cs) | 16 | **~31ms (약 32fps)** | "16ms ≈ 60fps 추종" ← **사실과 다름** |
| `AnimationFrameMs` | [`App/Config/DefaultConfig.cs:84`](../../App/Config/DefaultConfig.cs) | 16 | **~31ms (약 32fps)** | "애니메이션 프레임 간격 (~60fps)" ← **사실과 다름** |
| `CursorMotionPollMs` | [`App/Config/DefaultConfig.cs:328`](../../App/Config/DefaultConfig.cs) | 50 | ~62ms | "정지 검출 모드 — 50ms" (실제 4 틱) |
| `CapsLockPollMs` | [`App/Config/DefaultConfig.cs:87`](../../App/Config/DefaultConfig.cs) | 200 | ~203ms | 무해 (13 틱, 오차 1.5%) |

`AnimationFrameMs` 는 **메인 인디 애니메이션 전체의 프레임 상수**다 —
[`Core/Animation/OverlayAnimator.cs`](../../Core/Animation/OverlayAnimator.cs) 의 Fade 4 지점(`:256`,
`:321`, `:419`, `:510`) + Highlight(`:552`) + Slide(`:621`), 그리고
[`App/UI/CursorOverlay.cs:322`](../../App/UI/CursorOverlay.cs) 의 커서 IME 전환 팝.
**즉 이 앱의 모든 애니메이션이 60fps 가 아니라 약 32fps 로 돌고 있다.**
...
## 미착수로 남긴 것 (사용자 미승인 — 코드 변경 0)

1. **`CursorAlwaysPollMs` 16 → 15, `AnimationFrameMs` 16 → 15.**
   효과: 배달 주기 30.78ms → 15.56ms(약 2배 개선). 800px/s 로 마우스를 움직일 때 커서 링이 화면상
   **약 12px 더 붙는다.** 신규 config 키 0, [`Core/`](../../Core) 변경 0줄, 값 리터럴 2개 수정.
2. **위 표의 "사실과 다름" 주석 2곳 정정** — `DefaultConfig.cs:83` 과 `:330`.
   **1번 코드 변경과 반드시 같은 커밋에서 처리할 것** (주석만 먼저 고치면 다음 사람이 "그럼 왜 16
   그대로 두었나"를 다시 조사하게 된다).
