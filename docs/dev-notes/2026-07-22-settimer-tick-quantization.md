...
`AnimationFrameMs` 는 **플로팅 배지 애니메이션 전체의 프레임 상수**다 —
...
그려 합성을 우회하지만, 레이어드 윈도우인 커서 헤일로는 DWM 합성을 거친다. 따라서 폴링을 아무리
...
- 커서 헤일로 엔진이 왜 플로팅 배지와 별개인지 / `WH_MOUSE_LL`·`WM_INPUT` 를 왜 거부했는지:
