# 2026-06-02 파일 탐색기에서 플로팅 배지 사라짐 — 시스템 필터 HIDE 디바운스

## 결론

사용자 보고 "파일 탐색기를 클릭하면 플로팅 배지가 깜박이다 사라진다 — **애니메이션 ON 일 때만**"
...
**회귀 #2/#3 ("부팅 직후 플로팅 배지 잠깐 보였다 사라짐")** 과 동일 계열이다. 당시 가설 E/F
...
| 증상 | 플로팅 배지가 깜박이다 사라진 채 박제 (SW_HIDE + Hidden) |
