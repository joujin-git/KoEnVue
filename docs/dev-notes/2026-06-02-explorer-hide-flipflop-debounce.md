...
- `HandlePositionUpdated`: `wasHidden` / `fgChanged` 필드 추가.
- `HandleImeStateChanged`: `Main indicator show via IME change` 로그 추가.

이 race 영역은 무로깅이면 육안 검증에만 의존하게 되므로(2026-05-20 의 반복 실패 원인 중
하나), Show 경로 진입 사유를 로그로 남겨 다음 진단을 돕는다.
