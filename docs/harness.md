...
**이게 뭐예요?** Claude Code 는 명령줄에서 도는 AI 보조 도구입니다. "하네스" 는 그 도구가 KoEnVue 프로젝트에서 일관되게 동작하도록 잡아주는 설정 묶음 — 모델은 Opus(2026-07-30부터 **빠른 출력 모드는 끔** — 속도보다 품질 우선), 작업 깊이는 상황에 맞춰 조정(데스크탑 앱 실효 effort `xhigh`), 코드 바꾸면 문서 동기화 안내, 큰 작업만 여러 AI 가 나눠서 교차검증(ultracode 수동 호출), 세션 끝나면 자동 저장 등.

**왜 필요해요?** 매번 같은 설명을 다시 안 해도 되고, 다른 장비로 옮겨도 작업이 이어집니다. 잊을 만한 안전망(자동 wip 커밋, 비밀번호 마스킹) 도 자동으로 잡습니다.

**처음 써보기 — 5단계**:
1. **Claude Code 실행** — 터미널은 `claude`, 데스크탑 앱은 이 프로젝트 폴더를 열면 됩니다. 활성 판별은 **표면 무관한 기준**으로: 세션 시작 컨텍스트에 `[harness] KoEnVue 하네스 활성화` 문구가 주입됐는지(SessionStart hook 산출물). 터미널에서는 화면 아래 statusLine 에 `[opus · high] | git:main* | 한/En 하네스` 도 함께 보이지만, **데스크탑 앱에서는 이 커스텀 statusLine 이 표시되지 않습니다**(2026-07-29 화면 확인). 앱은 대신 자체 배지(`권한 무시` / `Opus 5 · 빠름` / `엑스트라`)를 보여줍니다 — 다만 이 관측은 **2026-07-29 시점(당시 `fastMode: true`)** 이라, `빠름` 배지는 **07-30 fast mode 해제 후 사라져야 정상**입니다(§1 모델 행). 앱이 `effortLevel` 을 무시한 전례가 있어 `fastMode` 존중 여부도 보장되지 않으니, 이 배지가 해제의 실효를 눈으로 확인하는 런타임 신호입니다 — 계속 `빠름` 이면 앱이 이 설정도 무시하는 것. `엑스트라`(=`xhigh`)가 설정값 `high` 와 어긋나는 것은 이미 확정(§1 Effort 행)
...
| 결정 | 내용 | 이유 |
|------|------|------|
| 모델 | `opus` alias (최신 Opus 자동 추종 — 버전 숫자를 박지 않는다) + **`fastMode: false`** | 최고 성능 모델을 **기본 출력 경로로** 사용. ⚠️ **2026-07-30 사용자 결정으로 fast mode 해제** — 2026-07-24 재구성 때 속도를 위해 켰으나(모델 다운그레이드는 아니었음), 응답 품질을 우선해 끔. 키 자체는 남겨 `false` 로 **명시**한다(삭제하면 "검토한 적 없음"과 구분되지 않음 — `/harness-status` grep 도 이 키를 읽는다). model/effort 는 statusLine payload(`payload.effort.level`)로 전달됨 — 미수신 시 statusline.ps1 이 env→`high` 폴백. **데스크탑 앱에선 statusLine 자체가 렌더되지 않아 이 경로가 무실효**(§10) |
...
- **`-FallbackContext`/`-EventName` 안전망**: `Write-HookOutput` 직전에 죽어도 catch 경로가 최소 fallback 컨텍스트("effort(데스크탑 앱 실측 xhigh) + thinking(fast mode 미사용), 큰 작업만 워크플로우" + 이전 세션 포인터) 1줄을 주입 — 인용한 문구는 **SessionStart** 것이고, **PreCompact 는 별도 문구**("대화 압축됨 — 진행 작업 연속성 확인(git status·docs/sessions 최신). effort 유지, 큰 작업만 워크플로우.")를 씁니다. 즉 `-FallbackContext` 를 쓰는 곳이 2곳이지 문구가 공유되는 건 아닙니다(재구성 전엔 삭제된 inject-turn-context 포함 3곳)
...
7. **additionalContext — 같은 내용은 한 번만**: doc-sync 대상·워크플로우 경고·push 실패를 `hookSpecificOutput.additionalContext` 로 내보낸다. **실측(2026-09-17, 데스크탑 앱 2.1.271): Stop 의 additionalContext 는 "다음 사용자 턴에 노출"이 아니라 그 자리에서 모델을 다시 깨운다.** 깨어난 턴이 끝나면 hook 이 또 돌고 트리는 여전히 dirty 라 같은 문구가 또 나가, **커밋될 때까지 빈 턴이 반복**됐다(백그라운드 에이전트 대기 중 약 10회 — 재구성 당시 "스모크로 노출 여부 확인" 으로 미뤄 둔 자리가 이 형태로 드러났다). 그래서 ① 입력의 `stop_hook_active` 가 참(hook 이 일으킨 연속 턴)이면 보내지 않고 ② 직전에 보낸 것과 **지문**(보낼 문구 + 리마인더 규칙에 걸린 파일 목록 — 세션 로그·docs 편집처럼 규칙 밖 파일은 제외)이 같으면 보내지 않는다. 보낼 것이 없어지면(커밋 등) 지문(`.claude/state/stop-context.last`)을 지워 다음 변경에 다시 한 번 알린다. 매 실행의 판정(`emitted` / `skip-duplicate` / `skip-stop-hook-active` / `reset` / `nothing-to-send`)과 `stop_hook_active` 수신 여부는 `.claude/state/stop-last.json` 에 남는다 — "안 보냈다"가 중복 억제인지 hook 무동작인지 가르는 런타임 흔적. 검증: 임시 git 저장소에서 실제 스크립트로 7상황 + 수정 전 원본 대조군(같은 상태 반복 시 매번 발송 = 루프 재현).
...
- **같은 프로젝트에서 다른 세션이 살아 있으면 wip 커밋 생략** (2026-09-17): `git add -A` 는 **다른 세션의 미커밋 변경**까지 담는다. 실제로 사용자가 연 짧은 곁 세션(`/resume-session` 후 삭제)의 SessionEnd 가 작업 중이던 메인 세션의 세션 로그를 wip 커밋·push 했다 — 그때는 로그뿐이라 무해했지만, 메인 세션이 코드를 고치다 멈춘 순간이었다면 **반쯤 된 코드가 main 에 push** 됐다. `_common.ps1` 의 `Get-OtherLiveSessions` 가 hook 입력의 `transcript_path` 폴더(slug 계산 안 함)에서 자기 기록을 뺀 `*.jsonl` 을 보고 판정한다 — `<id>.desktop-released.json`(데스크탑 앱의 해제 기록)이 있으면 종료, 비대화형(entrypoint `sdk*`, 예: `claude -p`)은 최근 `$HeadlessSessionLiveMinutes`(10분), 그 밖(데스크탑·터미널 대화형)은 사용자를 기다리며 오래 쉬므로 `$InteractiveSessionLiveMinutes`(120분) 안에 기록이 있으면 살아 있음. 생략하면 `session-end` 블록에 "**wip 커밋 생략** — 다른 세션이 작업 중: …" 을 남기고 커밋하지 않는다(push 는 그대로). 오판의 방향은 안전 쪽 — 살아 있다고 잘못 보면 wip 가 **미뤄질 뿐** 변경은 디스크에 남는다. `transcript_path` 가 없으면 판단 불가로 종전처럼 커밋. 판정(`clean` / `skip-other-live` / `committed` / `committed-sessions-unknown`)·다른 세션 목록은 `.claude/state/session-end-last.json` 에 남는다. 검증: 임시 git 저장소 + 가짜 세션 기록 폴더로 실제 스크립트 9상황(살아 있음·해제·오래됨·비대화형 창 안팎·다른 세션 없음·id 불일치·판단 불가·깨끗한 트리) + 수정 전 원본 대조군(같은 조건에서 wip 커밋 = 사고 재현).
...
| C: 기본 경로 (레거시) | `C:\Users\<user>\.claude\projects\E--dev-KoEnVue\memory\` — 이제 `Sync-Memory` 의 흡수/미러 대상일 뿐. 드라이브문자 대소문자는 Claude Code 버전에 따라 변동(2026-07 이전 `e--`, 이후 `E--`). Windows 는 대소문자 무시라 접근엔 무영향. **`Get-AutoMemoryDir` 는 설정값이 아니라 이 기본 경로를 계산한다** — 의도된 설계(흡수 대상 지정). slug 는 `:` `\` `/` 를 모두 `-` 로 치환(`/` 는 2026-09-17 추가 — 아래 재발 기록) |
...
| `Sync-Memory` 실제 발화 | ✅ 발동 조건 재현 검증 (2026-07-29) — 실패/복구/정상 3상태 전부: 실패 시 `❌ 동기화 실패 N건` + `hook-errors.log` 기록, 부재 시 `created` 경고 + `restored=10`, 정상 시 침묵. E:↔C: 해시 불일치 0. **2026-09-17 위치 가드 추가 후 재검증** — 슬래시/백슬래시/대문자 슬래시 경로가 같은 정위치로 수렴, 가드 발화(❌) · 수정 전 원본 대조군(중첩 오배치 재현) 확인 |
...
수정 2점: ① slug 정규식에 `/` 추가(`'[:\\/]'`). ② `Sync-Memory` 에 **위치 가드** — `memory` 의 조부모가 `projects` 자체인지 `GetFullPath` 정규화 후 비교하고, 아니면 **아무것도 만들지 않고** `errors` 로 보고(→ `❌ 동기화 실패` + `hook-errors.log`). `projects` 경로는 `Get-ClaudeProjectsDir` 한 곳에서 계산. 검증은 실제 `session-start.ps1` 을 가짜 `USERPROFILE`·가짜 프로젝트로 end-to-end 실행: 수정본은 소문자 슬래시(C: 부재) → `created` + 14건 정위치(깊이 2, 해시 불일치 0) · 같은 프로필에 백슬래시·대문자 슬래시 → 침묵(같은 디렉토리로 수렴) · **변이본**(가드 유지, 정규식만 옛것) → `❌ C: 경로 계산 이상` + 디렉토리 0개 + 로그 기록 · **원본 대조군** → 중첩 오배치 재현. 잘못 생긴 `e-\dev\KoEnVue` 는 휴지통으로 이동 — 사전 해시 대조 관문이 PowerShell 함수 출력 오염(문자열이 bool 반환값에 섞여 배열 → 항상 truthy)으로 **무력했음이 이동 후 드러나**, 휴지통의 실파일을 E: 와 **사후 대조**해 14/14 일치 확인(같은 `e-\dev\` 아래 `QuantPilot` 은 다른 프로젝트라 보존 — 같은 slug 결함을 공유했을 가능성 있음).
...
4. SessionStart hook 컨텍스트와 함께 사용자 프로필/규칙 복원. `created=$true` 면 "C: 복원/초기화 감지" 경고가 함께 표시됨 — 복구 건수와 함께 그 다음 줄의 `C: 경로:` 가 `projects` 바로 아래의 `E--dev-KoEnVue\memory`(드라이브문자 대소문자 변동은 무관)인지 확인할 것. 건수만으로는 중첩 오배치(`projects\e-\dev\…`)를 알아볼 수 없음 — 위 2026-09-17 재발 기록
