...
`TrayIcon` 의 `DrawCaretDot`/`DrawStrikeThrough` 는 같은 prologue/finally 보일러(`CreateSolidBrush` + `GetStockObject(NULL_PEN)` 셀렉트 → 그리기 → 원래 brush/pen 복원 + `DeleteObject`)를 `private readonly ref struct SolidFillScope` (using 패턴) 로 추출해 공유한다 (AUDIT DUP-12). 생성자가 solid brush 와 NULL_PEN 을 셀렉트하고 `Dispose` 가 pen/brush 를 복원한 뒤 brush 핸들만 해제 — 위와 동일하게 NULL_PEN(stock) 은 복원만 하고 삭제하지 않으며, 정리 대상은 `CreateSolidBrush` 핸들뿐이다. `readonly ref struct` 라 스택 전용(힙 할당 0·람다 클로저 0) 으로 NativeAOT 린 앱에 부합. 두 그리기 함수는 `using var _ = new SolidFillScope(hdc, fgColor);` 후 좌표 산술 + `Rectangle`/`Ellipse` 만 수행한다 — GDI 호출 순서·좌표·픽셀 산출물은 추출 전과 동일.
