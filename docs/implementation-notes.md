...
- **`TriggerPop()`** — `SetImeState` 가 가시 상태 + **`AnimationEnabled && CursorChangeHighlight`** 일 때 호출 (마스터 게이팅 — 아래 PR-22 항목 참조). `_popActive=true` + `_popStartTick=TickCount64` + `SetTimer(_hwndTimer, TIMER_ID_CURSOR_POP, AnimationFrameMs=16ms)` + **즉시 `HandleCursorPopTimer()` 1회 호출** (16ms 대기 없이 시작 배율로 첫 프레임 렌더). 가시 상태에서 IME 가 실제로 바뀌면 첫 프레임이 새 색 + 시작 배율로 렌더되므로 별도 색 갱신 `Render` 불요 — 마스터 OFF 또는 `CursorChangeHighlight` OFF 일 때만 색만 즉시 `Render`.
...
#### Cross-monitor 셸 무시 (제3 케이스, PR-24)

`TryHandleFilter` 진입부에는 `ResolveForApp`/`ShouldHide` 평가 **이전**에 동작하는 제3 케이스 "무시(ignore)" 게이트가 있다 — filtered 도 not-filtered 도 아닌 제3 상태. 멀티모니터에서 다른 모니터의 작업표시줄(`Shell_TrayWnd`)이 잠깐 foreground 를 훔치면 조건 4(클래스 블랙리스트)가 HIDE 를 내는데, 작업표시줄이 인디와 **다른 모니터**에 있으면 인디를 가리지도 않으므로 숨길 이유가 없다(조건 4 는 "가림 여부" 를 보지 않아 cross-monitor 에서 오탐). 게이트는 세 조건 AND — ① `DetectionState.LastNonFilteredForeground != Zero`(인디가 떠 있던 anchor 존재) ② `SystemFilter.IsMonitorScopedShell(fgClass)`(foreground 가 작업표시줄 = `DefaultConfig.MonitorScopedShellClasses` 매칭, `MatchesAny` 재사용 P4) ③ `!SystemFilter.SameMonitor(fg, anchor)`(작업표시줄이 인디 anchor 와 다른 모니터, `MonitorFromWindow` NEAREST 핸들 비교) — 충족 시 HIDE 미발신 + `DetectionState` 전부 불변 + `return true`(tick 종료 → `EmitStateChanges` 미도달 → 위치/IME/focus 갱신 0)로 현 인디를 동결한다. **`ShouldHide` 는 무변경.** anchor `LastNonFilteredForeground` 는 인디가 떠 있던 직전 non-filtered 앱(position_mode `window`/`fixed` 양쪽에서 인디 배치 기준)으로, non-filtered 확정 분기에서 갱신되고 잠정 HIDE 구간에선 위 디바운스의 "상태 미갱신" 비대칭과 함께 동결된다. 게이트가 `ShouldHide` 조건 4 의 fall-through 가 **아닌** 이유 — 조건 4 에서 false 를 반환하면 작업표시줄이 not-filtered 로 취급돼 ⓐ `WM_POSITION_UPDATED(작업표시줄)` 로 `position_mode=window` 인디가 모니터2 로 점프하고 ⓑ anchor 가 작업표시줄로 오염돼 다음 틱 `SameMonitor` 가 같은 모니터로 판정→게이트 자기 무력화. 그래서 "숨기지도 not-filtered 로 만들지도 않는" 제3 상태가 필수. 바탕화면(`Progman`/`WorkerW`)은 전체 데스크톱을 덮어 "다른 모니터" 개념이 없어 `MonitorScopedShellClasses` 에서 제외 — 조건 4 로 항상 숨김. 상세: [PR-24-cross-monitor-shell-hide.md](improvement-plan/PR-24-cross-monitor-shell-hide.md) + [dev-notes/2026-06-05-cross-monitor-shell-hide.md](dev-notes/2026-06-05-cross-monitor-shell-hide.md).
