...
`TrayIcon` 은 GDI 도형 API (`Rectangle`/`Ellipse`) 를 아예 쓰지 않는다 — `PaintIcon` 이 32bpp DIB 픽셀에 직접 쓰므로 brush/pen 을 만들 일이 없고, 정리 대상 GDI 핸들은 DIB section·mask bitmap·memory DC 뿐이다 (`CreateIcon` 의 `finally`). 브러시/펜 prologue-finally 보일러를 공유하던 `private readonly ref struct SolidFillScope` (AUDIT DUP-12) 는 이 전환으로 사용처가 0 이 되어 함께 제거됐다.
...
- **항상 표시 모드 (디폴트, `CursorAlwaysShow = true`)** — 타이머가 `CursorAlwaysPollMs = 15` 로 빠르게 호출 (`SetTimer` 양자화로 실배달 ~15.6ms≈64fps; 이전 16은 ~31ms≈32fps — [dev-notes/2026-07-22-settimer-tick-quantization.md](dev-notes/2026-07-22-settimer-tick-quantization.md)). 이동/정지 분류 자체를 skip 하고 매 tick `RenderAtCursor` 만 호출 → cursor 위치 추종. 숨기지 않음

**커서 표시 (PR-29/30/31, AlwaysShow 전용)**: `cursor_display_mode` — **`soft`**(기본)=흐릿하게 · **`sharp`**=선명하게 · **`motion`**=이동 중 흐릿하게. Motion은 맨해튼 δ > **`CursorMotionDimThresholdPx`(=1)** 즉시 딤, 연속 **`CursorMotionDimSettlePolls`(=8)** 틱 후 Full (enter/exit 동일 δ). 숨김 모드 Hide는 `CursorMotionThresholdPx`(=5). 안개 셰이더: σ≈`baseHalo`×(1+soft×(FogSigmaMul−1)), α 디폴트 **0.55**/softness **0.35**. IME 스케일 팝은 HighlightScale만 — Soft/딤 안개 유지. 구 `cursor_motion_dim_enabled` → soft 키 없을 때 true→motion / false→sharp 마이그. [`CursorMotionDim`](../App/UI/CursorMotionDim.cs) + `CursorOverlay`. [PR-31](improvement-plan/PR-31-cursor-display-mode.md).
...
The indicator is a separate TOPMOST window, not tied to any foreground window's geometry.
Client hit-test (`WM_NCHITTEST → HTCLIENT`) plus capture distinguishes short click vs drag:
short left-click temporarily hides the indicator (`_clickDismissed`); movement past
...
`config.drag_modifier` (`DragModifier` enum: `None` / `Ctrl` / `Alt` / `CtrlAlt`) gates whether
movement past the system drag threshold promotes to a native drag. The gate is checked on
`WM_MOUSEMOVE` while the overlay holds capture (`GetAsyncKeyState` for Ctrl/Alt).

- **None (default)** — threshold exceeded → always promote to `HTCAPTION` drag.
- **Ctrl / Alt / CtrlAlt** — promote only when the exact modifier chord is held. Otherwise the
  button-up path treats the gesture as a short click (temporary hide).
- **Short left-click (any `drag_modifier`)** — temporary hide; re-show on focus or IME change.

Key properties:

- Hot reload of `drag_modifier` costs nothing extra — the next pointer track reads `_config.DragModifier`.
- Once a drag begins, Windows enters a modal `WM_ENTERSIZEMOVE` loop with mouse capture.
- `Shift` is reserved for axis-lock during an active drag (see [`LayeredOverlayBase.HandleMoving`](../Core/Windowing/LayeredOverlayBase.cs)).
- **Class cursor must be `IDC_ARROW` explicit.** Without `hCursor`, the OS applies `IDC_APPSTARTING` over the client area during the per-process startup grace period. All `WNDCLASSEXW` registration goes through [`Win32DialogHelper.RegisterStandardClass`](../Core/Windowing/Win32DialogHelper.cs).
...
- **Fixed mode**: `config.default_indicator_position` (`DefaultPositionConfig` record) — `Corner` + `DeltaX` + `DeltaY` (logical px) resolved against the **foreground window's monitor work area** via `Overlay.ResolveAnchor(workArea, anchor, dpiScale)` (`dpiScale = DpiHelper.GetScale(hMonitor)`). Monitor from `MonitorFromWindow(..., MONITOR_DEFAULTTONEAREST)`.
- **Window mode**: `config.default_indicator_position_relative` (`RelativePositionConfig` record) — `Corner` + `DeltaX` + `DeltaY` (logical px) resolved against the **foreground window's DWM frame** via `Overlay.ResolveRelativePosition(frame, rel, dpiScale)`, then `ClampToVisibleArea` in `GetAppPositionWindow`
...
`Program.ClampToVisibleArea(x, y)` wraps absolute coordinates before they are shown. Resolves the target monitor via `DpiHelper.GetMonitorFromPoint(x + w/2, y + h/2)` with `MONITOR_DEFAULTTONEAREST` semantics, so a coordinate whose original monitor has been disconnected re-routes to the nearest surviving monitor's work area.
...
**Read path — stored value is never rewritten.** `GetAppPositionFixed` clamps hwnd-dict and `config.IndicatorPositions` returns only; `_hwndPositions` / `config.IndicatorPositions` entries retain their original values. Reattaching the original monitor restores the original position on the next lookup. Defends monitor removal / resolution change / DPI change scenarios that would otherwise leave the indicator unreachable.
...
- Drag save is skipped; early-return `Overlay.Show` does not call `ClampToVisibleArea` (intentional — position is ephemeral)
...
시스템 입력 프로세스(`StartMenuExperienceHost`, `SearchHost`, `SearchApp`)는 `SystemFilter` 블랙리스트에 의도적으로 포함되지 않으므로(인디케이터를 표시해야 하므로), 이들 UI가 ESC 등으로 닫힐 때 인디를 숨기는 별도 메커니즘이 `DetectionService`에 있다. 두 가지 닫힘 패턴이 경험적으로 확인됨:

**(A) HWND 유지 + DWM cloaked — `StartMenuExperienceHost`**
ESC 후 foreground HWND가 수 초간 유지되며 DWM cloaked 상태(`DWMWA_CLOAKED`)가 된다. `DetectionService`가 매 틱마다 `Dwmapi.IsCloaked(hwndForeground)`를 확인하여 cloaked이면 `WM_HIDE_INDICATOR`를 보내고 `continue`한다. 이후 OS가 foreground를 이전 앱으로 돌리면 다음 틱에서 정상 표시 경로를 탄다.
...
Ease-out cubic interpolation: `1 - (1 - t)^3` via `TIMER_ID_SLIDE`. All animation timers use `DefaultConfig.AnimationFrameMs = 15` (실배달 ~15.6ms≈64fps; 이전 16은 양자화로 ~31ms≈32fps — [dev-notes/2026-07-22-settimer-tick-quantization.md](dev-notes/2026-07-22-settimer-tick-quantization.md)).

**Slide + Highlight 합성 (종합 감사 ⑩)**: slide 트랙(위치만)과 highlight 트랙(위치 + stretch 크기)이 같은 레이어드 윈도우의 `UpdateLayeredWindow` 를 `AnimationFrameMs` 간격 last-writer-wins 로 번갈아 set 하면 base↔확대 크기 진동 + 위치 점프가 보였다. **중간 단계**에서는 `TryStartSlide(prevX, prevY, newX, newY, willHighlight)` 진입부 가드 `if (_highlightActive || willHighlight) return;` 로 강조가 진행/예정이면 슬라이드를 **보류**했으나, slide 를 생략하면 모니터 간 이동 등에서 인디가 출발 위치에 머무는 미관 문제가 있어 **합성(두 효과 동시 진행, blit 은 한 writer 가 전담)으로 재구현**했다. 핵심은 slide 진행 중(`_slideActive`)에는 slide 가 **위치만 추적**하고 highlight 가 **그리는 일을 전담**한다는 것:
...
감지 스레드 본문은 [`App/Detector/DetectionService`](../App/Detector/DetectionService.cs) (`RunLoop` / `ProcessDetectionTick`). Program 정적 상태는 [`DetectionHost`](../App/Detector/DetectionHost.cs) Func 콜백으로만 주입 — `Program.cs` 는 호스트 배선 + `new Thread(() => DetectionService.RunLoop(host))` 기동만.
...
Detection thread (80 ms poll):
  1. Every poll: ResolveForApp + SystemFilter.ShouldHide
     - Filter confirmed + indicator visible:       WM_HIDE_INDICATOR (level trigger on `_indicatorVisible`; `LastFiltered` for dedupe/`foregroundChanged` only — PR-26)
...
Main thread:
  ...
  Forced show paths (UserHidden unhide / second-instance Activate) → `TryShowIndicatorIfForegroundAllowed` (live FG + ShouldHide; PR-26)```

### Detection loop resilience

`DetectionService.RunLoop`의 while 본문은 `catch (Exception ex) when (ex is Win32Exception or InvalidOperationException or COMException or ArgumentException)` 로 래핑되어 단일 폴링 예외(예: `WindowProcessInfo.GetProcessName` 실패, UAC 전환 중 일시적 `COMException`)가 감지 스레드를 종료시키지 않는다. 로직 버그(`NullReferenceException` 등) 는 이 필터에 걸리지 않아 표면화된다. `Thread.Sleep`은 try 밖에 위치하여 예외 후에도 폴링 간격이 유지된다. `_stopping` 필드는 `volatile`로 선언되어 `OnProcessExit`에서의 쓰기가 감지 스레드(`host.IsStopping()`)에서 즉시 가시적이다.
...
**IME 감지 무영향**: `ImeStatus.Detect(hwndFocus, threadId)` 의 `threadId` 는 `hwndForeground` 로 독립 결정되므로(`DetectionService` 폴링 경로), 본 폴백은 IME Tier 3(`GetKeyboardLayout`) 경로를 건드리지 않는다.
...
**대상별 차이 — `OverlaySuppressTarget` (2026-09-17)**: 감지 틱·Forced Show 게이트는 `Badge`, 커서 `IsOverShellUi` 는 `CursorHalo` 로 호출한다 (종전 `bool includeSystemInputProcesses` 대체). `CursorHalo` 만 `IsSystemInputProcess`(Start/Search) 를 포함하고, **`Badge` 는 바탕화면·작업 표시줄 클래스(`DefaultConfig.IsDesktopTaskbarClass` — private 배열 `DesktopTaskbarClasses` = `Progman`/`WorkerW`/`Shell_TrayWnd`/`Shell_SecondaryTrayWnd`)를 클래스 매칭에서 제외** — 포인터가 그 위를 지나가기만 해도 한/영 배지가 사라지던 것을 막는다. 예외는 클래스 매칭만 건너뛰므로 ① FG 축은 불변 — 그 표면을 클릭해 포커스가 넘어가면 위 조건 4 가 히스테리시스를 거쳐 숨김 ② `system_hide_processes_user` 프로세스 규칙은 그대로 적용 ③ `#32768`·셸 팝업 클래스·`SystemHideProcesses` 위에서는 배지도 계속 숨김 ④ 예외 목록은 고정 정책이라 `system_hide_classes(_user)` 편집으로 끌 수 없음. `DefaultSystemHideClasses` 는 이 배열을 spread 로 포함해 기본값 내용·순서 불변.
...
- **규칙** — *기준선 대비 이번에 값이 달라진 키는 앱이 이기고, 나머지는 디스크가 이긴다.* 기준선이 "앱이 디스크와 같다고 아는 상태" 이므로 그 대비 변경분이 앱의 의도이고, 그 외 디스크와의 차이는 사용자 편집이다. 중첩 객체는 재귀해야 그 안의 부분 편집이 살아남고, 디스크에만 있는 키(손으로 추가한 프로필·미래 키)는 그대로 보존한다.
- **병합 불가 시** — 기준선 없음(첫 저장)·디스크 읽기/파싱 실패·최상위 비객체면 앱 값을 그대로 쓴다. 저장 자체를 포기하면 사용자의 트레이 조작이 조용히 무시되므로 그쪽이 더 나쁘다.

- **되읽기 실패 시** — 병합 후 `TryLoad` 가 실패하면(백신·에디터·동기화 클라이언트의 순간 잠금) 디스크와 기준선에는 병합 결과가, 호출자가 받는 인메모리에는 병합 **전** 값이 남는다. 이 상태를 "동기화됨" 으로 두면 다음 저장이 두 번 배신한다 — 병합을 통째로 건너뛰거나(mtime 일치), 병합하더라도 기준선이 병합 결과라 사용자 편집 필드를 "앱이 이번에 바꿨다" 로 오분류한다. 그래서 `_lastMtime`·`_syncedMtime`·`_lastPersistedJson` 세 표식을 함께 **물린다** — 기준선은 호출자가 실제로 들고 있는 값으로 되돌리고, self-bump 를 취소해 5초 폴러가 핫리로드로 따라잡게 한다(그 `TryLoad` 성공이 넷을 한 번에 정상화). 확정 #19, `SaveMergeTests` 3종이 대조군으로 고정.

값 비교는 `JsonElement.GetRawText()` 문자열 비교로 충분하다 — 기준선과 현재 값 모두 같은 직렬화기가 만든 것이라 공백·키 순서가 동일하고, 이 판정은 "앱이 이 필드를 건드렸나"만 답하면 된다.

### 병합으로 들어온 값도 핫리로드와 같게 적용한다 (bug-hunt 2026-08-02 G6)

3-way 병합이 일어났다는 것은 **디스크에서 앱이 모르던 설정이 들어왔다**는 뜻이고, 이는 핫리로드와 본질적으로 같은 사건이다. 그런데 `Save` 의 mtime self-bump 가 5초 폴러를 잠재우므로 `HandleConfigChanged` 는 돌지 않는다 — 저장 경로가 그 자리를 대신하지 않으면 **어디서도 처리되지 않는다.**

이전에는 `SaveAndSync` 가 프로필 캐시·I18n·감지 방식·오버레이 엔진 **4가지만** 다시 세웠고, 커서 헤일로·트레이·로거·표시 전이는 각 호출자가 자기 맥락으로 이미 적용한 뒤였다 — **병합 전 값으로**. 헬퍼 주석은 "각 호출자가 자기 전이 판정으로 이미 수행" 을 근거로 들었지만, 그 판정 자체가 병합 전 값 위에서 끝난 것이라 전제가 성립하지 않았다. 결과는 화면과 설정의 영구 불일치였다(자기치유 경로도 self-bump 로 막혀 있다). 확정 #28·#31·#47.

- `ApplyConfigTransition(prev, next)` 가 설정 교체 반영의 **단일 구현**이다 — 로거 레벨/재초기화·I18n·프로필 캐시·감지 방식·오버레이 엔진·커서 헤일로·`user_hidden` 전이·트레이·`overlay_class_name` 경고.
- 진입점은 둘뿐 — `HandleConfigChanged`(핫리로드)와 `SaveAndSync`(병합). [conventions.md](conventions.md) 의 P6 invariant grep 두 줄이 이 구조를 고정한다.
- 전이 판정이 전부 `prev`/`next` 비교라 **두 번 호출해도 안전하다.** 저장 호출자가 자기 변경을 먼저 적용한 뒤 병합이 겹쳐 들어오면, 두 번째 호출은 실제로 달라진 것만 처리한다.
- `SaveAndSync` 는 **반환값이 없고 `_config` 를 직접 갱신한다.** 이전 시그니처는 병합 결과를 반환했고 호출자가 대입을 빠뜨리면 조용히 손실됐다 — 릴리즈 리뷰 2026-08-01 확정 #1 의 정체가 정확히 그 미대입이다. 주석으로 경고하는 대신 함정 자체를 없앴다.
- `_config` 는 `ApplyConfigTransition` **호출 전에** 새 값이어야 한다 — `ApplyCursorConfigChange` 와 `ApplyTrayEnabledTransition` 이 파라미터가 아니라 필드를 직접 읽는다.
...
- `Settings` → `Tray.OpenConfigFile()` — **일반 권한**: `notepad.exe` + quoted path (`UriLauncher`). Notepad is hard-coded instead of dispatching through the `.json` default handler because typical end-user machines have no `.json` association and the default path triggers Windows' "choose an app" dialog or silent no-op. Notepad ships on every Windows SKU, displays UTF-8 JSON correctly, and its save is picked up by the mtime poller so hot reload still applies after the user edits. **elevated (`AdminElevation.IsCurrentProcessElevated`)**: `explorer.exe /select,"{path}"` — Medium IL 탐색기에서 파일만 강조(선택). High IL notepad 가 후속 편집·자식 프로세스에 권한을 전파하던 PR-03 B5 잔여 표면을 닫음 (감사 M2).
- `None` — no-op.

`HandleTrayToggle` advances the visibility cycle via `Tray.ComputeLeftClickCycle(_config)` (pure — sets `UserHidden` + `CursorIndicatorEnabled` for the next of the four stages; see [Tray.LeftClick.cs](../App/UI/Tray.LeftClick.cs)), calls `Settings.Save(_config)` immediately (so the state survives restart), calls `ApplyCursorConfigChange()` **only when `CursorIndicatorEnabled` actually flipped** (the halo window lifecycle — a direct call is required because `Settings.Save`'s mtime self-bump suppresses `WM_CONFIG_CHANGED`), rebuilds the tray icon via `Tray.UpdateState` (so the drawn shapes match the new stage), and delegates the badge overlay transition to the shared `ApplyUserHiddenTransition(wasHidden, isHidden)` helper — which no-ops unless `UserHidden` itself changed:

- `UserHidden: false → true` — invokes `HideOverlay()` (`Animation.TriggerHide(forceHidden: true)` + sets `_indicatorVisible = false` + clears system-input frame cache).
- `UserHidden: true → false` — does **not** unconditionally Show. Calls `TryShowIndicatorIfForegroundAllowed` (PR-26): live `GetForegroundWindow` + `ResolveFocusWindow` + `ResolveForApp` + `SystemFilter.ShouldHide`; only then `ShowIndicatorAtForeground`. If the live foreground is filtered (desktop / tray / `enabled:false` profile / etc.), the show is skipped — the detection thread will Show on the next non-filtered tick via `WM_POSITION_UPDATED`. The same helper is used by `HandleActivateRequest` (second-instance Activate).
...
- **`ModalDialogLoop.RejectReentry()` — 재진입 판정 단일 진실원 (AUDIT-2026-07-30 §A, 2026-08-01)** — 활성 모달이 있으면 그 창을 포그라운드로 끌어내고 `true` 를 반환한다. 호출처는 **다이얼로그 3종의 `Show()` 첫 문장** + `Tray.ShowMenu` + `DialogShell.Run` 5곳.

  **왜 `DialogShell.Run` 안의 가드만으로는 부족했나** — 각 `Show()` 는 `Run` 을 부르기 **전에** 자기 정적 상태를 리셋하고(`CleanupDialog._items = items`, `SettingsDialog._workingConfig = config` + `_fields.Clear()`, `ScaleInputDialog._scaleDlgClosed = false`), `Run` 이 재진입으로 `false` 를 돌려줘도 그 뒤 에필로그가 그대로 실행돼 상태를 마무리로 무효화한다(`_items = null!`, `_hwndScaleEdit = 0`). 즉 **가드가 있었는데도 첫 다이얼로그의 상태가 두 번째 호출에 의해 파괴**됐고, 살아 있던 창에서 「확인」을 누르면 `CommitSelections` 의 `_items` 참조가 `NullReferenceException` → WndProc 이 `[UnmanagedCallersOnly]` 라 관리 예외가 `DispatchMessageW` 경계를 넘어 **NativeAOT 가 프로세스를 종료**했다. 따라서 판정은 **상태를 건드리기 전**이어야 하며, 이 위치 조건은 grep 으로 검증되지 않으므로 `ModalReentryGuardTests` 가 정적 필드 보존으로 고정한다(가드를 프롤로그 뒤로 옮기면 3케이스가 실패).

  **모달 중에 재진입이 성립한 이유** — `ModalDialogLoop.Run` 의 `EnableWindow(hwndOwner, false)` 는 마우스·키보드 입력만 막고, 중첩 루프의 `GetMessageW(out msg, IntPtr.Zero, 0, 0)` 는 필터가 없어 **스레드의 모든 창 메시지**를 받는다. 트레이 아이콘은 explorer 소유라 사용자 클릭이 계속 살아 있고, explorer 가 `hwndMain` 으로 post 한 `WM_TRAY_CALLBACK` 이 그대로 디스패치된다. 그래서 1차 방어를 `Tray.ShowMenu` 에 둔다 — 다이얼로그 3종의 진입 경로가 전부 이 메뉴이므로 여기서 막으면 재진입 자체가 없어지고, 모달 중 색상·투명도·종료 같은 다른 메뉴 명령이 실행되는 부수 위험도 함께 닫힌다.

  **센티넬 예외** — `RunExternal` 이 세우는 `ExternalModalSentinel`(-1) 은 실제 창이 아니므로 포커스 복원 대상에서 제외한다(외부 모달은 Win32 가 이미 포그라운드를 잡고 있다). 판정만 하고 복원은 건너뛰므로 `MessageBoxW` 표시 중에도 트레이 메뉴는 정상적으로 차단된다.
...
**왜 다른 메뉴 항목은 OR 안 함**: Snap / Animation / Cursor halo 등 다른 메뉴 항목은 모두 `config.*` 직접 반영 — 외부 환경 영향 받는 항목이 0. admin 항목만 **부모 셸 토큰 상속** 이라는 외부 환경 영향을 받는 유일한 케이스라 OR 정당. fix #4 는 메뉴 빌더 한 곳만 OR — 다른 메뉴 빌더에 OR 패턴 확산하지 않음.
...
**IL mismatch mutex (감사 M1)**: High IL 1st 인스턴스가 mutex 를 보유한 채 Medium 2nd 가 `Mutex` 생성자에 접근하면 `UnauthorizedAccessException` 이 난다 (`createdNew=false` 가 아니라 예외). [`Program.Bootstrap.TryAcquireMutex`](../Program.Bootstrap.cs) 가 이를 catch 해 `false` 반환 → 호출부가 동일하게 `NotifyExistingInstance()` 로 이어지게 한다 (소유권은 못 얻지만 기존 인스턴스 활성화 신호는 보냄).
