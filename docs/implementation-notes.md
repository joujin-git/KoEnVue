...
**이동 중 시인성 저하 (PR-29, AlwaysShow 전용)**: `CursorMotionDimEnabled`(디폴트 **true**) 이면 매 tick 맨해튼 δ > `CursorMotionThresholdPx` 로 "이동" 판정 → 즉시 딤, 정지는 **연속 `CursorMotionDimSettlePolls`(=3) 틱** 후 Full 복귀(enter/exit 동일 δ·exit만 히스테리시스). 딤은 **셰이더 가우시안 안개** — `MotionSoftness`&gt;0 이면 하드 코어 없이 σ≈`baseHalo`×`CursorMotionFogSigmaMul`(14), 색+흰색 혼합; `RingAlphaInner/Middle/Outer` = `cursor_motion_alpha`×배수 1.00/0.97/0.94 (디폴트 α=0.22); soft&gt;0 시 `MotionFogPadLogicalPx=28` 로 bbox 여유. CAPS ON=3원·OFF=2원. 창 `SourceConstantAlpha` 는 Full(`_displayAlpha`). IME 팝·딤 OFF 시 soft=0·원별 α=1. [`CursorMotionDim`](../App/UI/CursorMotionDim.cs) + `CursorOverlay` / `CursorRenderer`. 설계: [PR-29](improvement-plan/PR-29-cursor-motion-distraction-reduction.md).
