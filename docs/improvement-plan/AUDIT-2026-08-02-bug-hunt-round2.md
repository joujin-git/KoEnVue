...
**라운드 4·5 의 finder 8종이 전부 실패했다** (`hunt:shared-state#4` ~ `hunt:lifetime#5`). 사유는 **월 지출 한도 도달**.

07-30 감사도 같은 지점에서 끊겼고(그때는 세션 토큰 한도), 워크플로우는 두 번 모두 그것을 "새 결함 0" 으로 읽어 dry 카운터를 올리고 종료했다 — **수렴이 아니라 자원 소진이다.** 즉 이 목록도, 07-30 목록도 하한선이며, 같은 도구를 같은 예산으로 다시 돌리면 같은 자리에서 또 끊긴다.
