# PR-26: UserHidden 해제 시 SystemFilter 미재평가 — 필터 대상 위 플로팅 배지 잔류
...
트레이 좌클릭 또는 우클릭 메뉴의 **"플로팅 배지 숨김"(`user_hidden`)** 을 켰다가 다시 끄면, **원래 숨겨져야 할 창(바탕화면 `Progman`/`WorkerW`, 작업 표시줄 `Shell_TrayWnd` 등 SystemFilter 대상) 위에 플로팅 배지가 표시되고, 감지 스레드가 그것을 다시 숨기지 못한다.**
...
`TOTALCMD64` 사용 중 플로팅 배지 위치가 두 좌표를 **자동으로 왕복**한다:
...
- 커서 헤일로를 켠 뒤 왕복 간격이 1~2초 → 0.75초로 짧아짐(**상관만 확인, 인과 미검증**).
