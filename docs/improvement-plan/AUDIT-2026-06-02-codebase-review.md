...
| **DUP-11** 🔒 보류 | Med | `App/UI/Dialogs/SettingsDialog.Scroll.cs:79-100` ↔ `CleanupDialog.cs:320-339` | `ViewportProc`(WM_VSCROLL/WM_MOUSEWHEEL/default)가 동형 — 둘 다 helper에 위임만 | 🔒 **보류 (2026-06-02 검토)**: 스크롤 산술은 이미 `ScrollableDialogHelper` 로 단일화. 남은 건 dispatch 5줄(WM_VSCROLL 위임 + WM_MOUSEWHEEL + default)인데, 두 ViewportProc 모두 `[UnmanagedCallersOnly]` 정적이고 GCHandle 없이 hwnd 만 받으며 **각 클래스의 정적 필드(`_scrollPos`/`_scrollMax`/`_viewportClientH`/줄높이/`_hwndViewport`)에 분산 의존**. 단일 공유 WndProc 으로 합치려면 hwnd→상태 매핑 인프라 신설 필요 → 비용≫이득 | M |
| **DUP-12** 🔒 보류 | Low | `App/UI/TrayIcon.cs:151-179` ↔ `:185-209` | `DrawCaretDot`/`DrawStrikeThrough`가 `CreateSolidBrush→SelectObject→try/finally 복원+DeleteObject` GDI 보일러 동일 | 🔒 **보류 (2026-06-02 검토)**: `DrawCaretDot`/`DrawStrikeThrough` 의 prologue 4줄 + finally 3줄(brush/pen 스택 셋업·복원) 동일(14줄). 헬퍼화하려면 람다=매 그리기 클로저 할당(NativeAOT 린 앱에 부적합) 또는 ref struct using 가드(복잡도↑). 시각 산출물이라 자동검증 불가. 비용/위험 대비 이득 모호 | M |
...
| **HC-18** 🔒 보류 | Low | `SettingsDialog.Fields.cs:89,93,118,143,158` | `Math.Clamp(i,0,2/3/5/1)` enum 값 개수 매직넘버 (Combo 팩토리는 이미 `labels.Length` 클램프 → 일부 중복) | 🔒 **보류 (2026-06-02 검토)**: `labels.Length-1` 참조는 setIdx 람다 스코프 밖(인라인 배열)이라 11곳 구조 변경 필요. `Combo.Commit` 이 이미 `[0, labels.Length-1]` 로 클램프하므로 setIdx 내부 Clamp 는 **이중 안전망** — 제거는 §4 "방어 코드 보수적 유지" 정신과 상충. 각 매직넘버는 옆 `(EnumType)` 캐스트가 의미를 self-document. 컴파일타임 enum 멤버수 const 부재로 깔끔한 const화 불가 + Low 이득 대비 11곳 변경 과다 | M |
...
| **IMP-6** 🔒 보류 | Low | `Program.cs:1105-1110` | `HandleDpiChanged`가 wParam/lParam 받지만 둘 다 무시 | 🔒 **보류 (2026-06-02 검토)**: wParam/lParam 무시하나 주석(HIWORD=newDpiY 등)이 **미래 per-monitor DPI 구현의 문서화된 hook**(이 앱은 멀티모니터 대응 중). 제거 이득(인자 2개) 미미 + WndProc dispatch 일관성 유지 | S |
