...
| **N** ✅ | 단건 9종 (아래 §N) — **8건 수정 · 1건 오탐 확정** | 13·34·39·42·48·50·55·59·60 (9) | ~~P2~~~~P3~~ **해결 2026-08-01** | 2/3 투표 → 메인 재확인 |
...
| # | 파일 | 요지 |
|---|------|------|
| 13 ✅ | `App/Startup/StartupTaskManager.cs` | ~~부팅 후 ~8초 백그라운드 schtasks 동기화와 트레이 토글이 같은 태스크를 락 없이 동시 수정~~ **해결** — 수정 경로 3곳(토글·경로동기화·admin 재등록)을 `_taskMutationLock` 으로 직렬화. 조회는 잠그지 않는다(메뉴 열 때마다 호출) |
| 34 ✅ | `Program.cs` | ~~애니메이션 타이머가 오버레이를 숨길 때 `_indicatorVisible` 미갱신 → 메인·감지 로직이 영구 불일치~~ **해결** — `Animation.Initialize` 에 `onHidden` 훅 추가, 애니메이터가 스스로 숨긴 경로에서도 플래그를 내린다 |
| 39 ✅ | `Core/Windowing/LayeredCursorBase.cs` | ~~DIB 섹션이 `_memDC` 에 select 된 채 Dispose — GDI 객체 해제 순서~~ **해결** — 초벌엔 오탐으로 봤으나 `DibSectionFactory:59` 가 `SelectObject` 후 복원하지 않는 것을 확인. GDI 는 선택된 비트맵의 `DeleteObject` 를 거부하므로 핸들이 샜다. DC 를 먼저 삭제하도록 순서 교정 — **`LayeredOverlayBase` 도 같은 구조라 함께 수정**(감사는 커서 엔진만 지적) |
...
| 48 ✅ | `Core/Config/JsonSettingsManager.cs` | ~~모든 저장이 디스크 대조 없는 read-modify-write → 외부 편집 소실~~ **해결** — 3-way 병합 도입. 기준선(`_lastPersistedJson`)은 앱이 마지막으로 디스크와 동기화된 상태이고, 저장 시 디스크 mtime 이 그때와 다르면 **이번에 바뀐 키만** 디스크 내용 위에 얹는다. 규칙: 앱이 바꾼 필드는 앱이, 나머지는 디스크가 이긴다. 중첩 객체는 재귀 병합, 디스크에만 있는 키는 보존 |
| 50 ✅ | `App/Detector/ImeStatus.cs` | ~~WinEvent 콜백은 **global** detection_method 로 분류, 감지 루프는 per-app resolved 로 분류 → 같은 창을 다르게 판정~~ **해결** — App 이 per-app 해석기를 주입(`SetPerAppDetectionMethodResolver`), 콜백이 감지 루프와 같은 기준으로 판정. 미배선 시 global 폴백 |
| 55 ✅ | `Program.cs` | ~~같은 IME 전이 1회에 `WM_IME_STATE_CHANGED` 가 2회 도착하는데 수신부에 멱등 가드 없음~~ **해결** — 감지 루프와 WinEvent 콜백이 각각 post 하는 것을 확인. 같은 상태면 조기 반환하되, `_lastImeState` 초기값과 우연히 같은 첫 메시지를 삼키지 않도록 `_imeStateReceived` 래치 병용 |
...
### §N 진행 상황 (2026-08-01) — 완료

**9건 중 8건 수정 · 1건 오탐 확정.** 항목별 결과는 위 §N 표 참조.

착수 과정에서 뒤집힌 초벌 판단 둘을 남긴다 — 감사 결과를 그대로 믿지 말라는 §3 의 실례다.

- **39 는 오탐이 아니었다.** 커서 엔진에 `SelectObject` 호출이 없어 처음엔 오탐으로 봤으나, 실제 select 는 `DibSectionFactory` 안에서 일어나고 **복원하지 않는다.** 같은 구조인 `LayeredOverlayBase` 도 같은 결함이었고 감사는 그쪽을 지적하지 않았다.
- **60 은 실제로 오탐이었다.** 호출처에 이미 가드가 있다.

**48 은 설계 판단이 필요한 유일한 건이었고, 규칙을 다음과 같이 정해 처리했다** — *앱이 이번에 바꾼 필드는 앱이 이기고, 나머지는 디스크가 이긴다.* 기준선이 "앱이 마지막으로 디스크와 같다고 아는 상태" 이므로 기준선 대비 변경분이 앱의 의도이고, 그 외 디스크와의 차이는 사용자가 파일에 직접 넣은 편집이다. 병합할 수 없는 상황(기준선 없음·디스크 파싱 실패)에서는 앱 값을 그대로 쓴다 — 저장을 포기하면 사용자의 트레이 조작이 조용히 무시되므로 그쪽이 더 나쁘다.

### 현재 상태 (2026-08-01 갱신)

**보류 사유였던 §A 는 닫혔다** — 위 §1 표와 §A 절 참조. 문서 첫머리의 "보류한다"는 2026-07-30 시점의 결정 기록이며, 재개 조건 자체는 충족된 상태다.

**2026-08-01 기준 14그룹이 전부 해결됐다** — A~M 13그룹(묶음 1~6) + §N 단건 9종 중 8건 수정·1건 오탐 확정. P0 1건 · P1 3건 · P2 7건 · P3 2건 · 단건 8건.

작업 중 **원래 67건 목록에 없던 결함 1건**을 추가로 찾아 고쳤다 — `config.json` 최상위가 객체가 아니면(`null`·배열·스칼라) 프로세스가 종료되던 경로로, §G 회귀 테스트를 쓰다 드러났다. §2 의 커버리지 한계가 실재함을 보여주는 사례다.

**남은 것은 묶음 7(`/bug-hunt` 재개)뿐이다.** 라운드 4·5 가 자원 소진으로 탐색을 수행하지 못했으므로(§2), 이 목록은 여전히 하한선이다.
