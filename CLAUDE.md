...
`WM_NCHITTEST → HTCAPTION` enables native drag. `WM_ENTERSIZEMOVE`/`WM_EXITSIZEMOVE` tracks drag lifecycle. `WM_MOVING` handles both Shift-key axis locking and cross-monitor DPI change during drag — hold Shift to constrain movement to the dominant axis (horizontal or vertical) from the drag start point.
