...
Windows IME state indicator (한 / En / EN). C# 14 / .NET 10 + NativeAOT single exe (~4.9 MB). Zero external NuGet.
...
- **테스트 = csproj 명시**: `dotnet test tests/KoEnVue.Tests/KoEnVue.Tests.csproj` (슬래시 — 백슬래시는 Bash 가 먹음). repo 루트에서 인자 없이 `dotnet test` 하면 메인 csproj 를 잡아 0개 실행·exit 0 착시.
- **커밋 = 항상 푸시까지**: `git commit` 후 즉시 `git push`. Stop hook(턴 끝 1회)이 자동 처리 — wip 커밋도 동일.
